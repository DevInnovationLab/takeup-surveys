...
people_per_hh <-
  ...
  bind_rows() %>%
  select(country, sample_group, wp_intervention, av_size = mean, av_size_se = se)

people_per_hh_census <-
  hh_census %>%
  dplyr::filter(
    wp_func,
    wp_intervention != "Non-program",
...
total_people_hhsizecensus <- 
  hh_per_wp %>%
  left_join(people_per_hh_census) %>%
  # Calculate people per WP and its SE using delta method
  mutate(
    av_people = av_hhs * av_size,
    # delta method: SE(X*Y) = sqrt((Y^2 * SE_X^2) + (X^2 * SE_Y^2))
...
table <-
  ...
  add_header_above(
    c(
      ...
      "Percent of color wheel readings ≥ 0.2 ppm" = 3
    )
  ) %>%
...
hh_census %>%
  ...
  summarize(mean_hh_members = mean(hh_members, na.rm = TRUE))
```
