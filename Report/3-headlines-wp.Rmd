...
cl_wp_sample <-
  ...
  map(
    ~ mean_cis(
      outcomes = c("watertreat_dispcl", "disctcr_02", "discfcr_02"),
      design = svydesign(
        id = ~ village_id + wp_id, # cluster IDs
        data = .x,
        nest = TRUE
...
users_wp_country %>%
  ...
  select(
    wp_intervention, sampled = wps, hhs, pop, total, se
  ) %>%
  kable()

## Export Table
```{r, message=FALSE, warning=FALSE}
users_wp    <- read_csv(here("../output/tables/users-wp.csv"))
chlorine_wp <- read_csv(here("../output/tables/chlorine-wp.csv"))

pick_hhs <- function(df) {
  if ("households" %in% names(df)) df$households else df$hhs
}

users_sum <- users_wp %>%
  group_by(country, wp_intervention) %>%
  slice_max(order_by = wps, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  mutate(
    hhs_any = pick_hhs(cur_data_all())
  ) %>%
  transmute(
    country,
    wp_intervention,
...
cl_sum <- chlorine_wp %>%
  ...
  select(country, wp_intervention, outcome, mean, ci, wps_chl, hhs_chl) %>%
  pivot_wider(
    id_cols     = c(country, wp_intervention),
    names_from  = outcome,                 
    values_from = c(mean, ci, wps_chl, hhs_chl),
...
for (i in seq_len(nrow(tab))) {
  r <- tab[i,]
  # country header when it changes
  ...
  line1 <- glue(
"\\hspace{{1em}}{prog} & {fmt_int(r$wps_hh)} & {fmt_int(r$hhs_hh)} & {fmt_int(r$vil_country)} & {fmt_int(r$people_tot)} & {fmt_int(r$disctcr_02_wps_chl)} & {fmt_int(r$disctcr_02_hhs_chl)} & {tcr_mean} & {fcr_mean} \\\\"
  )
  line2 <- glue(
...
writeLines(
  rows,
  here("../output/tables/headlines-wp.tex")
)
```
