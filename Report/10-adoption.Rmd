...
plot_data <-
  list(
    "(1)\nCensus sample,\nvillage-level\n(main estimate)\n" = cl_village_noweights_country %>% rename(N = hhs),
    "(2)\nCensus sample,\nwater point-level\n" = cl_wp_noweights  %>% rename(N = households),
    "(3)\nPromoter sample\n(Adoption Monitoring\nmethod)\n" = chlorine_am %>% rename(wp_intervention = wp_intervention_mon)  %>% rename(N = hhs)
  ) %>%
  map(
    ~ .x %>%
  ...
  mutate(
    ...
    label = label %>% paste0(" (N = ", N %>% format(big.mark = ","), ")") %>% str_trim
  ) %>%
  dplyr::filter(!(country == "Uganda" & wp_intervention == "ILC")) 
...
```{r}
col_names <- c("", "N", "Included in the promoter list", "N", "Water point with DSW/ILC is\nthe primary source of drinking water")

headers <- 
  c(
    " ",
    "Households using water points with DSW/ILC" = 2,
    "Households included in the promoter list" = 2
  )
...
plot_data <-
  ...
  bind_rows() %>%
  mutate(label = "(2)\nPromoter sample,\nconfirmed to be using water point\n") %>%
  bind_rows(
