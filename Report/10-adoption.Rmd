...
ggplot2::ggsave(
  filename = "output/appendix-graphs/adoption-comparison.pdf",  
  plot     = plot,            
  width    = 8,
  height   = 6,
  units    = "in",
...
```{r}
col_names <- c("", "N", "Included in\nthe promoter list", "N", "Water point with DSW/ILC is\nthe primary source of drinking water")

headers <- 
  c(
    " ",
    "Households using water\npoints with DSW/ILC" = 2,
    "Households in the\nPromoter Sample" = 2
  )
...
ggplot2::ggsave(
  filename = "output/appendix-graphs/adoption-cofirmed.pdf",  
  plot     = plot,            
  width    = 8,
  height   = 6,
  units    = "in",
...
ggplot2::ggsave(
  filename = "output/appendix-graphs/adoption-selection.pdf",  
  plot     = plot,            
  width    = 8,
  height   = 6,
  units    = "in",
...
footnote_text <- "All TCR readings were taken with a color wheel."
...
ggplot2::ggsave(
  filename = "output/appendix-graphs/adoption-promoter.pdf",  
  plot     = plot,            
  width    = 8,
  height   = 6,
  units    = "in",
...
ggplot2::ggsave(
  filename = "output/appendix-graphs/adoption-promotermon.pdf",  
  plot     = plot,            
  width    = 8,
  height   = 6,
  units    = "in",
...
ggplot2::ggsave(
  filename = "output/appendix-graphs/adoption-boundaries.pdf",  
  plot     = plot,            
  width    = 8,
  height   = 6,
  units    = "in",
...
```{r}
mon_survey_cl %>%
  dplyr::filter(wp_intervention_mon == "ILC") %>%
  mutate(
    confirmed = wp_intervention_mon == wp_intervention,
    primary = if_else(confirmed, str_detect(where_collect, "primary"), NA)
  ) %>%
  left_join(wp_census %>% select(wp_id, closer)) %>%
  group_by(country) %>%
  summarise(
    confirmed = mean(confirmed, na.rm = TRUE),
    primary = mean(primary, na.rm = TRUE),
    ilc_device = mean(primary & wp_ilc_device & confirmed, na.rm = TRUE),
    wp_fcr = mean(wp_discfcr_02, na.rm = TRUE),
    closer = mean(closer, na.rm = TRUE),
    tcr = mean(disctcr_02, na.rm = TRUE)
  )
```
...
hh_survey %>%
  ...
  left_join(wp_census %>% select(wp_id, closer)) %>%
  mutate(primary = str_detect(where_collect, "primary")) %>%
  summarise(
    primary = mean(primary, na.rm = TRUE),
    wp_ilc_device = mean(wp_ilc_device & primary, na.rm = TRUE),
    wp_fcr = mean(wp_discfcr_02, na.rm = TRUE),
    closer = mean(closer, na.rm = TRUE),
    tcr = mean(disctcr_02, na.rm = TRUE)
  )
...
mon_survey_cl %>%
  dplyr::filter(
    ...
    wp_intervention == "ILC",
    str_detect(where_collect, "primary")
  ) %>%
  summarise(
    n = n(),
    wps = n_distinct(wp_id),
    wp_ilc_device = mean(wp_ilc_device, na.rm = TRUE),
    tcr = mean(disctcr_02, na.rm = TRUE)
  )
```
