...
plot_data <-
  list(
    ...
    "Promoter sample\n(Adoption Monitoring\nmethod)" = total_country %>% rename(total = total_people, ub = total_people_ub, lb = total_people_lb, wp_intervention = wp_intervention_mon)
  ) %>%
  bind_rows(.id = "label") %>%
  mutate(
...
plot <-
  ...
  facet_grid(wp_intervention ~ country, scales = "free_y") + 
