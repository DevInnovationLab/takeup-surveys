...
footnote_text <- "Data on each water point with DSW/ILC was provided by Evidence Action. Village names were harmonized by us. Villages in Uganda that do are not part of DSW are excluded from the sample."

columns <- c(" ", paste0("(", 1:6,")"))

panels <- function(kable) {
  kable %>%
...
evac %>%
  ...
  add_header_above(columns, line = TRUE) %>%
  panels
```
...
table <-
  evac %>%
  select(-country) %>%
  mutate(ea_program_vil = str_replace_all(ea_program_vil, "&", "+")) %>%
  latex_table(
    align = "llccccc",
    caption = caption,
    label = label,
...
villages %>%
  ...
  summarise(
    ...
    across(
      c(dsw, ilc_wpt, ilc_wcp),
      ~ sum(.)
    )
  ) %>%
  html_table(
    col.names = c("Country", "Districts", "Villages", "Water points with DSW", "ILC water points", "ILC water collection points")
  ) %>%
  add_header_above(c("", paste0("(", 1:5, ")")))
```
...
sample_sizes %>%
  ungroup %>%
  select(-country) %>%
  html_table(col.names = c("Sample group", "Intervention", "Population of villages", "Sample villages")) %>%
  pack_intervention_se %>%
  add_header_above(paste0("(", 1:4, ")"))
```
...
### Number of program water sources per village
...
hh_census %>%
  ...
  mutate(nonprogram = !(wp_intervention %in% c("DSW", "ILC"))) %>%
  tabyl(wp_secweek, nonprogram) %>%
  adorn_totals
```
...
hh_census <-
  ...
  mutate(
    ...
    wp_nonprogram = !(wp_intervention %in% c("DSW", "ILC")),
...
hh_census %>%
  ...
  mutate(wp_intervention = if_else(wp_nonprogram, "None", as.character(wp_intervention))) %>%
  ...
  mutate(
    wp_intervention = 
      if_else(
        wp_nonprogram, 
        "None", 
        as.character(wp_intervention)
      ) %>%
      fct_relevel("None")
  ) %>%
  feols(
    wp_secweek ~ wp_intervention | country + sample_group,
    cluster = ~ village_id
  ) %>%
  summary
```
...
hh_survey %>%
  ...
  group_by(country, sample_group, wp_intervention) %>%
  mutate(primary = where_collect == "From the water point primary water point (${wp_name_pl})") %>%
  summarise(mean(primary, na.rm = TRUE))
```
