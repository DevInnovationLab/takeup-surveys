...
estimates <- table1 %>%
  ...
  summarise(table1 = sum(indiv_access_popn, na.rm = TRUE),
            lb_table1 = sum(lb, na.rm = TRUE),
            ub_table1 = sum(ub, na.rm = TRUE)) %>%
  left_join(
  ...
  left_join(
    table8 %>% group_by(country) %>% summarise(table8 = sum(indiv_access_popn, na.rm = TRUE),
                                                lb_table8 = sum(lb, na.rm = TRUE),
                                                ub_table8 = sum(ub, na.rm = TRUE)),
    by = "country"
  )
...
df <- df %>%
    mutate(amount_rounded = round(amount / 20000) * 20000,
           lb_rounded = round(lb / 20000) * 20000,
           ub_rounded = round(ub / 20000) * 20000)
...
# calculate positions
  df <- df %>%
    ...
    mutate(
      ...
      ci_lower = ifelse(id == 1 | id == n(), lb_rounded, NA),
      ci_upper = ifelse(id == 1 | id == n(), ub_rounded, NA)
    )
...
# create waterfall charts
  ggplot(df, aes(x = category, fill = type)) +
    geom_rect(aes(xmin = id - 0.45, xmax = id + 0.45, 
                  ymin = start, ymax = end)) +
    geom_errorbar(aes(x = id, ymin = ci_lower, ymax = ci_upper),
    ...
    scale_y_continuous(limits = c(0, max(df$ci_upper, na.rm = TRUE) * 1.1), 
                       labels = scales::comma)
}
...
lm_robust(disctcr_02 ~ survey,
          data = chlor,
          clusters = wp_id)

lm_robust(discfcr_02 ~ survey,
          data = chlor,
          clusters = wp_id)
```
