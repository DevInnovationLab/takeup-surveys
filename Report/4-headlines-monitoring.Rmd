...
table8 <- users_table %>%
  select(country, intervention, popn_users, se_popn_users, popn_users_lb, popn_users_ub) %>%
  mutate(
...
## Breaking down chlorination rates differences
```{r}
chlorination_data_weighted_combined <- hh_survey %>%
  mutate(intervention = wp_intervention) %>%
  select(-wp_intervention) %>%
...
household survey: 26.6, 29.0, 30.1

household survey & functional: 26.6, 27.1, 30.1 --> table 1 / 1

both surveys & functional: 27.7, 25.3, 34.1

------ WATER POINT SELECTION ------

both surveys & functional & promoter survey water points: 28.1, 25.7, 37.5

household survey & promoter survey water points: 27.4, 30.1, 33.8

------ HOUSEHOLD SELECTION - PROMOTER LIST ------

both surveys & functional & promoter survey water points & listed by promoter: 29.1, 20.9, 38.5

household survey & functional & promoter survey water points & listed by promoter: 28.6, 23.2, 34.1

------ HOUSEHOLD SELECTION - MONITORING SURVEY ------

monitoring survey & promoter survey water points & listed by promoter: 30.3, 17.9, 46.6

monitoring survey & functional: 30.8, 20.6, 46.0 --> table 3

possible factors:
1) selection of water points into promoter survey: TABLE 1 -> hh survey but only promoter survey water points
2) selection of households into promoter list: hh survey but only households in promoter list (& water points in promoter survey)
3) dataset (remainder) promoter list: only households in promoter list (& water points in promoter survey), but monitoring survey
...
table1_tcr <- calculate_mean_ci_chlorination(table1, "disctcr_02", 1)

table1_tcr
```

### 2) only pm survey WPs 
```{r}
table1_pmsurvey <- hh_survey %>%
  mutate(intervention = wp_intervention) %>%
  select(-wp_intervention) %>%
  dplyr::filter(survey == "Household Survey") %>%
  # dplyr::filter(survey == "Monitoring Survey") %>%
...
table1_tcr_pmsurvey <- calculate_mean_ci_chlorination(table1_pmsurvey, "disctcr_02", 1)
table1_tcr_pmsurvey
```

### 3) only pm survey WPs & pm list households 
```{r}
table1_pmlist <- hh_survey %>%
  mutate(intervention = wp_intervention) %>%
  select(-wp_intervention) %>%
  dplyr::filter(survey == "Household Survey") %>%
  # dplyr::filter(survey == "Monitoring Survey") %>%
...
table1_tcr_pmlist <- calculate_mean_ci_chlorination(table1_pmlist, "disctcr_02", 1)
table1_tcr_pmlist
```

### 4) monitoring survey, not hh survey
30.9, 20.6, 46
29.4, 27.4, 50.0 - rank < 5, monitoring survey
32.5, 21.7, 48.2 - rank > 5, monitoring survey
29.5, 24.2, 46.3 - rank < 5, survey - any
...
table3 <- hh_survey %>%
  ...
  # dplyr::filter(survey == "Household Survey") %>%
  dplyr::filter(survey == "Monitoring Survey") %>%
  # dplyr::filter(survey == "Monitoring Survey" & !is.na(rank)) %>%
  # dplyr::filter(survey == "Monitoring Survey" & as.numeric(rank) >= 5) %>%
  # dplyr::filter(as.numeric(rank) < 5) %>%
  # dplyr::filter(survey == "Household Survey" & as.numeric(rank) < 5 | survey == "Monitoring Survey") %>%
  # dplyr::filter(promoter_list == "TRUE") %>%
  # dplyr::filter(promoter_surveyed == "TRUE") %>%
  dplyr::filter(wp_func == "TRUE") %>%
  dplyr::filter(!is.na(intervention)) %>%
...
table3_tcr <- calculate_mean_ci_chlorination(table3, "disctcr_02", 1)
table3_tcr
```

### plot
```{r}
combined_tcr <- bind_rows(
  table1_tcr %>% mutate(sample = "1) Table 1: HH survey, \n all WPs"),
  table1_tcr_pmsurvey %>% mutate(sample = "2) HH survey, \n only HHs using \n PM survey WPs"),
  table1_tcr_pmlist %>% mutate(sample = "3) HH survey, \n PM listed HHs \n using PM survey WPs"),
  table3_tcr %>% mutate(sample = "4) Table 3: \n Monitoring survey")
)

ggplot(combined_tcr, aes(x = sample, y = mean, color = country, shape = intervention)) +
  ...
  labs(
    title = "TCR by Dataset & Sample Restriction",
    x = "Sample & dataset",
    y = "Proportion with TCR > 0.2",
    color = "Country",
    shape = "Intervention"
  ) +
  theme_minimal() +
...
#     rank_under5 = rank < 5)
# 
# malawi_hh_survey_DSW <- hh_survey %>%
#   dplyr::filter(country == "Malawi")  %>%
#   dplyr::filter(wp_intervention == "DSW") %>%
#   mutate(
#     rank_under5 = rank < 5)
# 
# malawi_hh_survey_ILC <- hh_survey %>%
#   dplyr::filter(country == "Malawi")  %>%
#   dplyr::filter(wp_intervention == "ILC") %>%
#   mutate(
#     rank_under5 = rank < 5)
# 
# uganda_model <- lm_robust(disctcr_02 ~ rank_under5 + survey, 
#                           data = uganda_hh_survey,
#                           clusters = wp_id)
# 
# malawi_model_DSW <- lm_robust(disctcr_02 ~ rank_under5 + survey, 
#                           data = malawi_hh_survey_DSW,
#                           clusters = wp_id)
# 
# malawi_model_ILC <- lm_robust(disctcr_02 ~ rank_under5 + survey, 
#                           data = malawi_hh_survey_ILC,
#                           clusters = wp_id)
# 
# # convert to regular lm objects for stargazer
# uganda_lm <- lm(disctcr_02 ~ rank_under5 + survey, data = uganda_hh_survey)
# malawi_lm_DSW <- lm(disctcr_02 ~ rank_under5 + survey, data = malawi_hh_survey_DSW)
# malawi_lm_ILC <- lm(disctcr_02 ~ rank_under5 + survey, data = malawi_hh_survey_ILC)
# 
# stargazer(uganda_lm, malawi_lm_DSW, malawi_lm_ILC,
#           type = "text",
#           title = "Association between Rank < 5, Survey and Chlorination",
#           dep.var.labels = "disctcr 02",
#           # covariate.labels = c("Listed by Promoter"),
#           column.labels = c("Uganda", "Malawi DSW", "Malawi ILC"),
#           model.numbers = FALSE,
#           se = list(uganda_model$std.error, malawi_model_DSW$std.error, malawi_model_ILC$std.error),
#           p = list(uganda_model$p.value, malawi_model_DSW$p.value, malawi_model_ILC$p.value),
#           keep.stat = c("n", "rsq", "adj.rsq"),
#           add.lines = list(c("Clustered SE", "Waterpoint", "Waterpoint","Waterpoint")))
```
...
#     rank_under5 = rank < 5)
# 
# uganda_model <- lm_robust(disctcr_02 ~ rank_under5, 
#                           data = uganda_hh_survey,
#                           clusters = wp_id)
# 
# malawi_model_DSW <- lm_robust(disctcr_02 ~ rank_under5, 
#                           data = malawi_hh_survey_DSW,
#                           clusters = wp_id)
# 
# malawi_model_ILC <- lm_robust(disctcr_02 ~ rank_under5, 
#                           data = malawi_hh_survey_ILC,
#                           clusters = wp_id)
# 
# # convert to regular lm objects for stargazer
# uganda_lm <- lm(disctcr_02 ~ rank_under5, data = uganda_hh_survey)
# malawi_lm_DSW <- lm(disctcr_02 ~ rank_under5, data = malawi_hh_survey_DSW)
# malawi_lm_ILC <- lm(disctcr_02 ~ rank_under5, data = malawi_hh_survey_ILC)
# 
# stargazer(uganda_lm, malawi_lm_DSW, malawi_lm_ILC,
#           type = "text",
#           title = "Association between Rank < 5 and Chlorination",
#           dep.var.labels = "disctcr 02",
#           # covariate.labels = c("Listed by Promoter"),
#           column.labels = c("Uganda", "Malawi DSW", "Malawi ILC"),
#           model.numbers = FALSE,
...
mt <- monitoring_table %>%
  select(country, intervention, n_wp, n_hh_count, pop_points,
  ...
  mutate(
    ...
    pop_points = if_else(
      country == "Malawi" & intervention == "ILC",
  ...
  mutate(
    ...
    intervention = factor(intervention, levels = c("DSW","ILC"))
  ) %>%
...
for (i in seq_len(nrow(mt))) {
  ...
  line1 <- glue(
"\\hspace{{1em}}{r$intervention} & {fmt_int(r$n_wp)} & {fmt_int(r$n_hh_count)} & {fmt_int(r$pop_points)} & {fmt_int(r$popn_users)} & {fmt_int(r$n_wp_chl)} & {fmt_int(r$n_hh)} & {tcr_mean} & {fcr_mean} \\\\"
  )
...
table1_exout <- table1 %>%
  left_join(
    in_village_share %>% select(country, intervention = wp_intervention, share_in_village),
    by = c("country", "intervention")
  ) %>%
  mutate(
  ...
  select(country, intervention, indiv_access_popn) #, lb, ub, se)

table1_exout

# And for the country totals
...
OLD version: I tried to first exclude households that are outside the village, adjust response rate to account for this, and find total. But I was running into many problems so settled for a simpler approach (above).
```{r}
# users_data_exout <-
#   hh_census %>%
#   dplyr::filter(
...
#   ) %>%
#   # Recalculate response rate for in-village households only
#   group_by(village_id) %>%
#   mutate(
#     response_rate = mean(response)  # Recalculate based on filtered data
#   ) %>%
#   ungroup() %>%
#   group_by(country, sample_group) %>%
#   mutate(
...
#   group_by(village_id, country, sample_group, wp_intervention) %>%
#   summarise(
#     response_rate = unique(response_rate),  # Now this is the correct rate
#     hhs = n(),
#     users = sum(hh_members)
#   ) %>%
#   mutate(
...
OLD (same reason as above for village level)
```{r}
# users_data <-
#   hh_census_wp %>%
#   dplyr::filter(
