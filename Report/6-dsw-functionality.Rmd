...
hh_survey %>%
  ...
  pivot_wider(
    id_cols = c(sample_group, est),
    values_from = c(hhs, disctcr_02, discfcr_02),
    names_from = country
  ) %>%
  mutate(
    across(
      c(sample_group, contains("hhs")),
  ...
  select(sample_group, contains("Malawi"), contains("Uganda")) %>%
  latex_table(
  ...
  add_header_above(c(" ", "Malawi" = 3, "Uganda" = 3), line = TRUE) %>%
