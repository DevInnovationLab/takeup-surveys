...
plot_data <-
  list(
    ...
    "Without correction for water point\nfunctionality" = users_vil_func,
    "Correcting for potentially missed \nwater points" = users_vil_misspoints
  ) %>%
    map(
      ~ .x %>%
...
plot_data <-
  list(
    ...
    "Including households with unconfirmed\nDSW/ILC status" = cl_village_all,
    "Including households with no stored water" = cl_village_nottested,
    "Excluding villages with unseasonable rain" = cl_village_rain,
    "Excluding villages with potential \npromoter interference" = cl_village_int
  ) %>%
    map(
      ~ .x %>%
