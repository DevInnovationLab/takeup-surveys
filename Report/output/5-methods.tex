...
We use the R package \textit{survey} to estimate the country-level reach and the confidence interval around it.\footnote{\textcite{survey}.} The point estimates are equivalent to the procedure described below, but the package also takes the survey design into account when calculating the standard errors (stratification by country and sample group, sampling weights, clustering at the village leval):
...
The chlorine treatment adoption rate is estimated by randomly sampling households who were matched to water points with DSW/ILC during the Household Census (the ``Census Sample'') and calculating the share of these households that had chlorine detected in the water sample tested during the Household Survey.
...
\subsection{Promoter Sample: approximating Adoption Monitoring estimates}\label{sec:methods-mon}
...
\begin{enumerate}
    ...
    \item Calculate the average household size among households in the sampled from the promoter list (the ``Promoter Sample'' of the Household Survey);
