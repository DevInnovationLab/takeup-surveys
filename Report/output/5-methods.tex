...
\begin{itemize}
    \item \textcolor{blue}{We include villages where no ILC water collection points were functional -- for example if they were replaced for this reason or if they also had water points with DSW -- in the calculation of the average number of households using water points with ILC in our sample, assigning zero ILC users to each of them.} 
    \item \textcolor{blue}{We exclude households using water points with DSW/ILC located outside of village boundaries from village-level estimates. This is to avoid double counting these households when multiplying the average number of users per village by the number of villages served in the country (that is, we assume this household would have been counted in the village where the water point it uses is located).}
    ...
    \item \textcolor{blue}{In Step 3, when using the water point as the unit of reference, we multiply the number of water points in the country by the functionality rate within each village group and intervention.}
\end{itemize}
...
\begin{itemize}
    ...
    \item We restrict the sample to households that had water available in the household at the time of the survey. This is a deviation from Evidence Action's Adoption Monitoring protocol, since they assume these households do not chlorinate. \textcolor{red}{Our sensitivity checks include this alternative version of the estimates.}
