...
Table \ref{tab:headlines-vil} shows our main estimates of reach and adoption. Tables \ref{tab:headlines-wp} and \ref{tab:headlines-am} present alternative estimates, the first one using the same data, but a different unit of reference, and the second approximating the Adoption Monitoring methods.

\textbf{Chlorine Residual detection rates are similar among households whose primary water source has a DSW across both countries and village groups, with 27\% to 33\% of households having TCR levels above 0.2 mg/L according to the color wheel reading}, even though the share of dispensers that had chlorine available varies more across the same divisions (between 48\% in Malawi Expansion and 80\% in Uganda in general). If we consider FCR as the main way to measure chlorine presence in the water, detection rates vary between 17\% and 22\%. \textbf{TCR detection rates among households water points with ILC are similiar, at 28.9\%}, and FCR detection rates are lower, at 13.8\%. 

\textcolor{red}{These rates make sense when taking into account that although ILC does not require the additional effort of treating the water or the discomfort of handling dilute chlorine, the results from the water point census indicate that ILC is harder to dose and to maintain, which means in the end the FCR at the water point is lower.}

Table \ref{tab:dsw-func} shows functionality rates for DSW dispensers in Malawi and Uganda, and Table \ref{tab:dsw-func} presents chlorine detection rates at the water points after treating the water with dilute chlorine solution from the dispenser. \textcolor{red}{Say that this seems to be fine and adoption is low because of filling and preferences.} Tables \ref{tab:ilc-func} and \ref{} show functionality and chlorine detection rates at ILC water points, respectively. It is harder to calculate access to chlorine, but say a back-of-the-envelope points to \%.

Reach estimates using the village and the water point are extremely similar, since they are basically taking unconditional averages of more or less the same households. Reach estimates are also similar, but lower when using the water point, which is expected given that the "catchment area" is incomplete. This points to water point being a viabel options adn cheaper.

\textcolor{red}{Using Adoption Monitoring methods, on the other hand, leader to more dissimilar results. similar population but 50\% chlorination rates in Uganda, and 50\% higher reach but similar chlorination rates in Malawi.
Sections \ref{sec:adoption} and \ref{sec:reach} explore potential exaplanitons for this.}
