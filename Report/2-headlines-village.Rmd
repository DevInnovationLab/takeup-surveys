...
```{r}
people_per_group %>%
  mutate(estimate = "by_group") %>%
  bind_rows(
    people_per_country %>%
      mutate(estimate = "by_country")
  ) %>%
  write_csv(
...
for (i in seq_len(nrow(tab))) {
  ...
  line1 <- glue(
"\hspace{1em}{prog} & {fmt_int(r$vil_hh)} & {fmt_int(r$hhs_hh)} & {fmt_int(r$vil_country)} & {fmt_int(r$people_tot)} & {fmt_int(r$disctcr_02_villages)} & {fmt_int(r$disctcr_02_hhs)} & {tcr_mean} & {fcr_mean}\\\\"
  )
