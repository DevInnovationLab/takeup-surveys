...
# creating group_cat
group_cat is the variable for water point type
```{r}
wp_census <- wp_census %>%
  mutate(
...
# create lookup table
wp_lookup <- wp_census %>% 
  select(wp_id, ea_id, country, sample_group, intervention_type, group_cat) %>%
  distinct()

wp_lookup_ea_id <- wp_census %>% 
  select(ea_id, country, sample_group, intervention_type, group_cat) %>%
  distinct(ea_id, .keep_all = TRUE) 

# join to other datasets
pm_survey <- pm_survey %>%
  select(-any_of(c("country", "sample_group", "group_cat", "ea_id"))) %>%
...
## section 7.3 - share of households in hh census identified by promoters
```{r}
hh_census %>%
...
## section 7.2 text - Uganda (sample: EA list versus promoter survey)
...
## section 7.2 text - Malawi (sample: EA list versus promoter survey)
...
## table 7.2: number of water points in ea list
...
## table 7.2: wp census (functional) and pm census
number of water points by type
```{r}
...
## table 7.3 (5.1 but using HH census and HH survey - only WPs in promoter survey)
```{r}
...
## table 7.4 caption: number of villages in pm survey and wp census
```{r}
...
## table 7.5 balance tables: Uganda DSW Footprint
...
## table 7.6 (balance): Uganda DSW Footprint but between EA list and not EA list water points
```{r}
...
## table 7.7 (balance):: Uganda DSW Expansion
...
## table 7.8 (balance):: Malawi DSW Footprint
...
## table 7.9 (balance):: Malawi DSW Expansion
...
## table 7.10 (balance):: Malawi DSW in ILC vill
no significant differences
```{r}
wp_census_pm <- wp_census %>%
  mutate(
...
# section 7.4.1 double-counting: how many hh ids come up more than once?
...
# section 7.4.1 double counting: number of water points used according to household census
...
## section 7.4.2: (7.3 but for all water points)
```{r}
wp_count_tbl_wp_census <- wp_census %>%
  # dplyr::filter(wp_id %in% pm_survey$wp_id) %>%
  group_by(country, group_cat) %>% 
...
# merge + calculations (same as in 5.1)
beneficiaries_tbl_wp_census_ALL <- wp_count_tbl_wp_census %>%
  left_join(hh_count_tbl_wp_census, by = c("country", "group_cat")) %>%
  left_join(hh_size_tbl_wp_census, by = c("country", "group_cat")) %>%
  left_join(chl_tbl_wp_census, by = c("country", "group_cat")) %>%
  left_join(country_totals, by = c("country", "group_cat")) %>%  
...
beneficiaries_tbl_wp_census_ALL
```

```{r}
country_level_estimates_wp_census <- beneficiaries_tbl_wp_census_ALL %>%
  group_by(country) %>%
  summarise(
    total_indiv_access_popn = sum(indiv_access_popn, na.rm = TRUE),
    # SE for total access: square root of sum of squared SEs (assuming independence)
...
## section 7.4.4 regression of chlorination on households using WPs in promoter survey:  water points in pm vs sample
```{r}
...
# promoter survey
model <- lm(disctcr ~ promoter_survey, data = uganda_hh_survey)
stargazer(model, 
          type = "text",   
          title = "Regression Results: Association between Promoter Survey and disctcr",
...
# ea list
model <- lm(disctcr ~ ea_list + promoter_survey, data = uganda_hh_survey)
stargazer(model, 
          type = "text",   
          title = "Regression Results: Association between EA list and disctcr",
          dep.var.labels = "DISCTCR",
          covariate.labels = c("EA list"),
          out = "regression_results.txt") 

model <- lm(disctcr ~ ea_list + promoter_survey, data = malawi_hh_survey)
stargazer(model, 
          type = "text",   
          title = "Regression Results: Association between EA list and disctcr",
          dep.var.labels = "DISCTCR",
...
## section 7.4.5 regression of chlorination on being listed by promoter
households listed by promoters vs others using water points in pm
I regress chlorination on household presence in the promoter lists, limiting the sample to households using water points in the promoter survey
- requires that you run the chunk above before to work
```{r}
hh_survey <- hh_survey %>%
  mutate(
...
are all households in HH census included in promoter list?

as

## exploratory - not directly used in report:
association between hh_listed by promoter and hh_count in hh census - it is positive and significant
```{r}
...
uganda_footprint_comparison <- pm_survey %>%
  dplyr::filter(country == "UGANDA" & group_cat == "DSW Footprint") %>%
...
regression_model <- lm(hh_counts_hhcensus ~ hh_listed, data = uganda_footprint_comparison)
