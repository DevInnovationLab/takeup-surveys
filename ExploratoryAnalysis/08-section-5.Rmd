...
```{r}
# manually add country totals, calculated in STATA
country_totals <- tibble::tribble(
  ~country, ~wp_category, ~country_total,
  "MALAWI", "DSW Footprint", 3123,
  "MALAWI", "DSW Expansion", 12040,
  "MALAWI", "DSW in ILC village", 1035,
  "MALAWI", "ILC", 2376,
  "UGANDA", "DSW Footprint", 4843,
  "UGANDA", "DSW Expansion", 12875
)
...
```{r}
# import table 1 numbers for each village type (sample_group)
country_totals_unadjusted <- tibble::tribble(
  ~country, ~sample_group, ~total_in_table_1,
...
table1_ex_outside <- country_totals_unadjusted %>%
  left_join(share_outside_tbl, by = c("country", "sample_group")) %>%
  mutate(
    indiv_access_popn = total_in_table_1 * (1 - coalesce(share_outside, 0))
  ) %>%
...
country_totals_table10_ex_out <- beneficiaries_tbl_wp_census %>%
  group_by(country) %>%
  summarise(
    total_indiv_access_popn = sum(indiv_access_popn, na.rm = TRUE),
    # SE: square root of sum of squared SEs (assuming independence)
...
# actual charts
```{r}
country_sums <- table1_ex_outside %>%
  group_by(country) %>%
...
>%
  left_join(
    table10_ex_outside %>% group_by(country) %>% summarise(table10 = sum(table10_ex_outside, na.rm = TRUE)),
    by = "country"
  ) %>%
  # ... continue with more joins
  mutate(
    value_exclude = table12 - table1,
    value_unit = table10 - table12
    # etc.
  )


value_table1 <- table1_ex_outside$indiv_access_popn 
value_exclude <- table12_ex_outside$indiv_access_popn - table1_ex_outside$indiv_access_popn  
value_unit <- table10_ex_outside$indiv_access_popn - table12_ex_outside$indiv_access_popn
value_sample <- table13$indiv_access_popn - table10_ex_outside$indiv_access_popn
value_double <- table14$indiv_access_popn - table13$indiv_access_popn
value_outside <- table13$indiv_access_popn - table8$indiv_access_popn
value_table8 <- table8$indiv_access_popn
...
#                "Add promoter\nsurvey and\nout-of-village\nhouseholds",
#                "Table 8"),
#   value = c(value_table1,
#             -value_exclude,  # make negative if it's a reduction
#             value_unit,
#             value_sample,
#             value_dataset,
#             value_add,
#             value_table8),
#   type = c("total", rep("relative", 5), "total")
# )
# 
...
#           calc_total = FALSE,
#           fill_by_sign = TRUE,
#           fill_colours = c("darkred", "steelblue"),
#           total_fill = "orange") +
#   geom_text(aes(label = value), vjust = -0.5, size = 4, color = "black") +
#   labs(title = "Breaking down the difference between census and monitoring\nestimates for Uganda",
