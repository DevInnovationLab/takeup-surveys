...
malawi_wp <- read_dta(
    file.path(
      path_box, 
      "Data/EvidenceAction/Final/malawi-water-points.dta"
    )
  ) %>%
  mutate(
...
villages <- 
  read_dta(
    file.path(
      path_box, 
      "/Data/EvidenceAction/Final/ea-villages.dta"
    ) 
  ) %>%
  mutate(
    villageid = as.character(villageid)
...
uganda_wp <- 
  read_dta(
    file.path(
      path_box, 
      "Data/EvidenceAction/Final/uganda-water-points.dta"
    ) 
  ) %>%
  mutate(
    villageid = as.character(villageid)
...
malawi_wcp <-
  read_dta(
    file.path(
      path_box,
      "Data/EvidenceAction/Final/malawi-water-collection-points.dta"
    )
  )
uganda_wcp <- 
  read_dta(
    file.path(
      path_box,
      "Data/EvidenceAction/Final/uganda-water-collection-points.dta"
    )
  )
```
...
country_totals <- bind_rows(malawi_wp, uganda_wp) %>%
  count(country, wp_category, name = "country_total_ex_wcp") %>%
  mutate(
    country_total = case_when(
      country == "MALAWI" & wp_category == "ILC" ~ country_total_ex_wcp + malawi_wcp_count, # add up water points and water collection points to get all ILC points
      country == "UGANDA" & wp_category == "ILC" ~ country_total_ex_wcp + uganda_wcp_count,
      TRUE ~ country_total_ex_wcp
    )
  ) %>%
...
### OLD moving from wp_category to sample_group and intervention_type
```{r}
table(wp_census$intervention_type, wp_census$sample_group)

table(wp_census$wp_category)
```
...
# MOVED test for difference between table 1 and 8
```{r}
# UGANDA
# t1 uganda
...
# t8 uganda
mean_t8_uganda <- country_totals_table8$total_indiv_access_popn[country_totals_table8$country == "UGANDA"]
ci_lower_t8_uganda <- country_totals_table8$ci_lower_total_access[country_totals_table8$country == "UGANDA"]
ci_upper_t8_uganda <- country_totals_table8$ci_upper_total_access[country_totals_table8$country == "UGANDA"]
...
# t8 malawi
mean_t8_malawi <- country_totals_table8$total_indiv_access_popn[country_totals_table8$country == "MALAWI"]
ci_lower_t8_malawi <- country_totals_table8$ci_lower_total_access[country_totals_table8$country == "MALAWI"]
ci_upper_t8_malawi <- country_totals_table8$ci_upper_total_access[country_totals_table8$country == "MALAWI"]
...
# MOVED test for difference between table 1 and 12
```{r}
#t12 uganda
mean_t12_uganda <- country_totals_table12$total_indiv_access_popn[country_totals_table12$country == "UGANDA"]
ci_lower_t12_uganda <- country_totals_table12$ci_lower_total_access[country_totals_table12$country == "UGANDA"]
ci_upper_t12_uganda <- country_totals_table12$ci_upper_total_access[country_totals_table12$country == "UGANDA"]
se_t12_uganda <- (ci_upper_t12_uganda - ci_lower_t12_uganda) / (2 * 1.96)

#t12 malawi
mean_t12_malawi <- country_totals_table12$total_indiv_access_popn[country_totals_table12$country == "MALAWI"]
ci_lower_t12_malawi <- country_totals_table12$ci_lower_total_access[country_totals_table12$country == "MALAWI"]
ci_upper_t12_malawi <- country_totals_table12$ci_upper_total_access[country_totals_table12$country == "MALAWI"]
...
## MOVED table 9 [numbers are currently off in cols 2-5]
...
## MOVED table 11
Number of households identified by different instruments
```{r}
...
difference in disp_filled not significant. difference in disp_tank_present is significant, but since it does not translate to significant difference in chlorine presence, I don't think this is important
```{r}
wp_census %>%
...
## MOVED table 17a
compare EA list with promoter survey
...
## MOVED section 6.4.1: adjusting numbers for waterfall charts
...
## MOVED tables excl outside HHs for waterfall charts
...
# breaking down differences in chlorination
```{r}
wp_selection_diff <- table12 %>%
  select(country, wp_category, chl_rate) %>%
  rename(chl_rate_t12 = chl_rate) %>%
  left_join(
    table10 %>% select(country, wp_category, chl_rate) %>% rename(chl_rate_t10 = chl_rate),
    by = c("country", "wp_category")
  ) %>%
  mutate(diff_wp_selection = chl_rate_t12 - chl_rate_t10)

hh_selection_diff <- table10 %>%
  select(country, wp_category, chl_rate) %>%
  rename(chl_rate_t10 = chl_rate) %>%
  left_join(
    table8 %>% select(country, wp_category, chl_rate) %>% rename(chl_rate_t8 = chl_rate),
    by = c("country", "wp_category")
  ) %>%
  mutate(diff_hh_selection = chl_rate_t10 - chl_rate_t8)

combined_comparison <- table12 %>%
  select(country, wp_category, chl_rate) %>%
  rename(chl_rate_t12 = chl_rate) %>%
  left_join(
    table10 %>% select(country, wp_category, chl_rate) %>% rename(chl_rate_t10 = chl_rate),
  ...
  mutate(
    diff_t12_to_t10 = chl_rate_t12 - chl_rate_t10,
    diff_t10_to_t8 = chl_rate_t10 - chl_rate_t8,
    diff_t12_to_t8 = chl_rate_t12 - chl_rate_t8
  )

print(combined_comparison)

# Reshape for grouped bar chart
chart_data_long <- combined_comparison %>%
  select(country, wp_category, diff_t12_to_t10, diff_t10_to_t8, diff_t12_to_t8) %>%
  pivot_longer(
  ...
  mutate(
    stage = factor(stage,
                   levels = c("diff_t12_to_t10", "diff_t10_to_t8", "diff_t12_to_t8"),
                   labels = c("WP Selection\n(T12→T10)", "HH Selection\n(T10→T8)", "Total Difference\n(T12→T8)"))
  )

ggplot(chart_data_long, aes(x = wp_category, y = change, fill = stage)) +
  geom_col(position = "dodge") +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  facet_wrap(~country, scales = "free_x") +
  scale_fill_brewer(palette = "Set2") +
  labs(
    title = "Breakdown of Chlorination Rate Differences by Selection Stage",
    x = "Water Point Category",
    y = "Difference in Chlorination Rate",
...
## MOVED section 6.2: sample: EA list versus promoter survey (Uganda) [numbers are currently off]
...
## MOVED section 6.2: sample: EA list versus promoter survey (Malawi) [numbers are currently off]
...
## MOVED section 6.3: share of households in hh census identified by promoters
...
## MOVED section 6.4.4: water points with some households outside of village (30%)
```{r}
...
# MOVED section 6.4.5 double-counting: how many hh ids come up more than once? [numbers are currently off - updated in description below, but not in report]
...
## MOVED section 6.4.6 regression of chlorination on being listed by promoter
...
# MOVED number of water points used per household - households in promoter lists
```{r}
...
## MOVED FCR instead of TCR
table 8 with FCR instead of TCR 
```{r}
# column 1: household count and SE
hh_count_tbl <- pm_survey  %>%
  mutate(hh_listed = as.numeric(hh_listed)) %>%
...
chl_tbl <- hh_survey %>%
  ...
  group_by(country, wp_category, wp_id) %>%
  summarise(
    wp_chl_rate = mean(discfcr > 0.2, na.rm = TRUE),
    wp_se_chl_rate = sqrt(wp_chl_rate * (1 - wp_chl_rate) / n()), # SE for proportion within WP
    .groups = "drop"
  ) %>%
  group_by(country, wp_category) %>%
...
# country totals and CIs
country_totals_table8_fcr <- table8_fcr %>%
  group_by(country) %>%
  summarise(
    total_indiv_access_popn = sum(indiv_access_popn, na.rm = TRUE),
    # SE: square root of sum of squared SEs (assuming independence)
...
country_totals_table8_fcr
```

table 10 with FCR instead of TCR
```{r}
# column 1: household count and SE
hh_count_tbl_wp_census <- hh_census %>%
  dplyr::filter(wp_id %in% pm_survey$wp_id) %>%
...
# merge + calculations
table10_fcr <- hh_count_tbl_wp_census %>%
  left_join(hh_size_tbl_wp_census, by = c("country", "wp_category")) %>%
  left_join(chl_tbl_wp_census, by = c("country", "wp_category")) %>%
  left_join(country_totals, by = c("country", "wp_category")) %>% # this is created in the "table 8" chunk
  mutate(
...
country_totals_table10_fcr <- table10_fcr %>%
  group_by(country) %>%
  summarise(
    total_indiv_access_popn = sum(indiv_access_popn, na.rm = TRUE),
    # SE: square root of sum of squared SEs (assuming independence)
...
country_totals_table10_fcr
```

# differences in HH size - difference in means test
```{r}
comparison <- table8 %>%
  select(country, wp_category, avg_hh_size_table8 = avg_hh_size, se_hh_size_table8 = se_hh_size) %>%
  left_join(
    table10 %>% select(country, wp_category, avg_hh_size_table10 = avg_hh_size, se_hh_size_table10 = se_hh_size),
    by = c("country", "wp_category")
  ) %>%
  mutate(
    diff = avg_hh_size_table8 - avg_hh_size_table10,
    se_diff = sqrt(se_hh_size_table8^2 + se_hh_size_table10^2),
    z_stat = diff / se_diff,
    p_value = 2 * pnorm(-abs(z_stat)),
    significant = p_value < 0.05
  ) %>%
  select(country, wp_category, 
         avg_hh_size_table8, avg_hh_size_table10, diff,
         se_diff, z_stat, p_value, significant)
...
# diff in chlorination
```{r}
hh_survey_filtered <- hh_survey %>%
  dplyr::filter(wp_category %in% c("DSW Expansion", "DSW Footprint")) %>%
  dplyr::filter(survey == "Monitoring Survey") %>%
  dplyr::filter(wp_id %in% pm_survey$wp_id) %>%
  # First aggregate to water point level
  group_by(country, wp_category, wp_id, village_id) %>%
  summarise(
    wp_chl_rate = mean(disctcr > 0.2, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(expansion_dummy = ifelse(wp_category == "DSW Expansion", 1, 0))

# Uganda regression - use regular lm
uganda_data <- hh_survey_filtered %>% filter(country == "UGANDA")
uganda_reg_lm <- lm(wp_chl_rate ~ expansion_dummy, data = uganda_data)

# Malawi regression - use regular lm
malawi_data <- hh_survey_filtered %>% filter(country == "MALAWI")
malawi_reg_lm <- lm(wp_chl_rate ~ expansion_dummy, data = malawi_data)

# Get cluster-robust standard errors using lm_robust for reference
uganda_reg_robust <- lm_robust(wp_chl_rate ~ expansion_dummy,
                               data = uganda_data,
                               clusters = village_id,
                               se_type = "CR2")

malawi_reg_robust <- lm_robust(wp_chl_rate ~ expansion_dummy,
                               data = malawi_data,
                               clusters = village_id,
                               se_type = "CR2")

# Extract cluster-robust SEs
uganda_se <- uganda_reg_robust$std.error
malawi_se <- malawi_reg_robust$std.error

# Use stargazer with custom SEs
library(stargazer)
stargazer(uganda_reg_lm, malawi_reg_lm,
          type = "text",
          se = list(uganda_se, malawi_se),
          title = "Chlorination Regressions by Country",
          column.labels = c("Uganda", "Malawi"),
          covariate.labels = c("DSW Expansion (vs. Footprint)"),
          dep.var.labels = "Chlorination Level (disctcr)",
          keep.stat = c("n", "rsq"),
          star.cutoffs = c(0.05, 0.01, 0.001),
          notes = "Standard errors clustered at village level in parentheses")
```

## regression of chlorination on sample group
```{r}
hh_survey_filtered <- hh_survey %>%
  dplyr::filter(sample_group %in% c("Expansion", "Footprint")) %>%
  dplyr::filter(intervention == "DSW") %>%
  dplyr::filter(survey == "Monitoring Survey") %>%
  dplyr::filter(promoter_surveyed == TRUE) %>%
  # First aggregate to water point level
  group_by(country, sample_group, wp_id, village_id) %>%
  summarise(
    wp_chl_rate = mean(disctcr > 0.2, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(expansion_dummy = ifelse(sample_group == "Expansion", 1, 0))

# Uganda regression - use regular lm
uganda_data <- hh_survey_filtered %>% dplyr::filter(country == "Uganda")
uganda_reg_lm <- lm(wp_chl_rate ~ expansion_dummy, data = uganda_data)

# Malawi regression - use regular lm
malawi_data <- hh_survey_filtered %>% dplyr::filter(country == "MALAWI")
malawi_reg_lm <- lm(wp_chl_rate ~ expansion_dummy, data = malawi_data)
...
### new
```{r}
hh_survey_chlorine <- hh_survey %>%
  dplyr::filter(survey == "Household Survey") %>%
  mutate(
    chlorination = ifelse(disctcr > 0.2, 1, 0)  # Create binary indicator for each household
  )

# Run regression with cluster-robust standard errors
library(estimatr)
regression_model_robust <- lm_robust(
  chlorination ~ hh_members, 
  data = hh_survey_chlorine,
  clusters = village_id,
  se_type = "CR2"
)

# For stargazer, run regular lm and extract clustered SEs
regression_model <- lm(disctcr ~ hh_members, data = hh_survey_chlorine)

# Extract cluster-robust SEs from lm_robust
# clustered_se <- regression_model_robust$std.error

# Use stargazer with clustered SEs
stargazer(regression_model,
          type = "text",
          # se = list(clustered_se),
          title = "Household Size and Chlorination",
          dep.var.labels = "Chlorination (TCR > 0.2)",
          covariate.labels = c("HH members"),
...
chlorination rates with different filters
```{r}
tests <- hh_survey %>%
  # dplyr::filter(survey == "Monitoring Survey") %>%
  dplyr::filter(survey == "Household Survey") %>%
  # dplyr::filter(wp_id %in% pm_survey$wp_id) %>%
  # dplyr::filter(hh_dsw == 1 | hh_ilc == 1)
  dplyr::filter(hh_dsw == 0 & hh_ilc == 0)
...
exploring self-reports as compared to intervention_type - I see no difference
```{r}
table <- hh_survey %>%
  dplyr::filter(hh_dsw == 0 & hh_ilc == 0) %>%
  # dplyr::filter(intervention_type == "Non-program")
...
table(hh_survey$hh_dsw)

table(hh_survey$intervention_type)

table(hh_survey$hh_ilc, hh_survey$intervention_type, useNA = "always")
```
