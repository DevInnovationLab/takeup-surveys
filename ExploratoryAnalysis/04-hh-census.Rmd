...
summary_data <- hh_valid %>%
  st_drop_geometry() %>%
  mutate(
    used_cl = rowSums(select(., all_of(wtmt_anycl)) == 1, na.rm = TRUE) > 0,
    category = case_when(
  ...
  count(category, wtmt_anycl, name = "n") %>%
  group_by(category) %>%
  mutate(
    total_n = sum(n),
    pct = round(100 * n / total_n, 1),
    label = ifelse(wtmt_anycl, "Used Treatment", "Did Not Use Treatment"),
...
ggplot(summary_data, aes(x = label, y = pct, fill = label)) +
  ...
  aes(
    label = paste0(pct, "%"),
    vjust = ifelse(category == "Village: Both DSW & ILC" & wtmt_anycl, 1.5, -0.4),
    color = ifelse(category == "Village: Both DSW & ILC" & wtmt_anycl, "white", "black")
  ),
  size = 3.5,
  show.legend = FALSE
  ) +
...
summary_data <- hh_valid %>%
  ...
  mutate(
    total_n = sum(n),
    pct = round(100 * n / total_n, 1),
    label = ifelse(wtmt_anycl, "Used Chlorine Treatment", "Did Not Use Chlorine Treatment"),
    category_label = paste0(category, "\n(n = ", total_n, ")")
  )
ggplot(summary_data, aes(x = label, y = pct, fill = label)) +
  geom_col(width = 0.6) +
