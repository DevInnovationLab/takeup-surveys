...
## Number of observations and number/share of households using DSW/ILC water points
...
We have the same thing in hh_survey, but I realized there is also a census measure for this
...
LW:
- characteristics of households that apply chlorine at home: 
  - household composition (younger children, pregnant women, maybe number of HHs in compound) -> DONE
  - whether in DSW or ILC villages
...
## Household composition of households applying chlorine at home
No clear trend here
```{r}
hh_census <- hh_census %>%
  mutate(
...
comp_data <- bind_rows(all_households, chlorine_users) %>%
  pivot_longer(
...
ggplot(comp_data, aes(x = Characteristic, y = Percent, fill = group)) +
  ...
  labs(
    title = "Household Composition: All vs Chlorine-at-Home Households",
    x = "",
    y = "Share of Households (%)",
    fill = "Household Type",
    caption = paste0(
      "Note: N = ", all_households$N, " for all households and N = ", chlorine_users$N,
      " for subset of households that applies chlorine at home."
    )
  ) +
...
In line with the fact that ILC water should already be treated
```{r}
hh_census %>%
  dplyr::filter(wtmt_method__111 == 1) %>%
  dplyr::mutate(
  ...
  dplyr::count(source_type, name = "n", sort = TRUE) %>%
  dplyr::mutate(
    pct = round(100 * n / sum(n), 1)
  ) %>%
  ggplot(aes(x = reorder(source_type, -pct), y = pct)) +
  geom_col(fill = viridis(1)) +
  geom_text(aes(label = paste0(pct, "%")), vjust = -0.3, size = 3.5) +
  labs(
    title = "Reported Village Water Source Type for Households Reporting\n'Nothing (Water Already Treated)'",
    x = "Village Water Source Type",
    y = "Share of Households (%)",
    caption = paste0("Note: Among ", 
                     sum(hh_census$wtmt_method__111 == 1, na.rm = TRUE),
                     " households that report 'Nothing (water is already treated)'.")
  ) +
  theme +
  theme(
...
## Share of households not using DSW or ILC who report using DSW or ILC 
