...
LW: different percentages now that we don't have wp_dsw_pl
```{r}
hh_census %>%
  st_drop_geometry() %>% 
  mutate(
...
secwhen_used %>%
  ...
  labs(
    ...
    caption = paste0("Note: In HH census raw data, there are ",total_households_census," households.\n",
                     ...
                     "could report more than one reason. *Added later (August).")
  ) +
  theme
```
...
wtmt_methods <- c(
  ...
  "wtmt_sand", "wtmt_solar", "wtmt_decant", "wtmt_other"
)

hh_valid <- hh_census[rowSums(!is.na(hh_census[, wtmt_methods])) > 0, ]

summary_data <- hh_valid %>%
  
  mutate(
    used_any = rowSums(select(., all_of(wtmt_methods)) == 1, na.rm = TRUE) > 0
  ) %>%
...
summary_data <- hh_valid %>%
  ...
  mutate(
    ...
    category = case_when(
      vil_dsw_selfreport == 1 & vil_ilc_selfreport != 1 ~ "DSW Only",
      vil_ilc_selfreport == 1 & vil_dsw_selfreport != 1 ~ "ILC Only",
      vil_dsw_selfreport == 1 & vil_ilc_selfreport == 1 ~ "Both DSW & ILC",
      TRUE ~ "Neither"
    )
  ) %>%
...
ggplot(wtmt_summary, aes(x = type, y = share, fill = type)) +
  ...
  labs(
    ...
    caption = paste0(
      "Note: N=", 
      nrow(dplyr::filter(hh_census, !is.na(vil_dsw_selfreport) | !is.na(vil_ilc_selfreport))),
      " households that report not using a DSW or ILC water source."
    )
  ) +
  theme
