...
hh_census %>%
  ...
  summarise(
    ...
    `Households (total)` = n(),
    `Households using Evidence Action water points` = sum(evac, na.rm = TRUE),
    `Avg HHs per village` = `Households (total)`/`Villages (n)`,
    `EvAc share (%)` = (`Households using Evidence Action water points`/`Households (total)`)* 100 %>% round(1)
  ) %>%
  kable(
    digits = 1,
