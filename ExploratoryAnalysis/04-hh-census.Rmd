...
n_households <- hh_census %>%
  filter(rowSums(!is.na(select(., wp_secwhen_1, wp_secwhen_2, wp_secwhen_3, wp_secwhen_4, wp_secwhen__666))) > 0) %>%
  nrow()
...
secwhen_used %>%
  ...
  coord_flip() +
  labs(
    title = "Share of Households Reporting Reasons for Using Other Water Sources",
    x = "Reason",
    y = "Share of Households (%)",
    caption = paste0("Note: ", n_households,
...
ggplot(wp_distribution_census, aes(x = "", y = n, fill = as.factor(wp_secweekno))) +
  geom_col(width = 1) +
  coord_polar(theta = "y") +
  geom_text(
    aes(label = label),
...
ggplot(summary_data, aes(x = "", y = pct, fill = label)) +
  ...
  labs(
    ...
    caption = paste0("Note: Among ", sum(summary_data$n), 
                     " households that report not using a DSW or ILC water source.")
  ) +
  theme +
  theme(
...
hh_census %>%
  ...
  kable(digits = 1, caption = "Percentage Share of Households with at Least One Young Child")
```
...
hh_census %>%
  ...
  kable(
    ...
    caption = paste0(
      ...
      n_demo_households,
      " households which had at least one non-missing value for any demographic field."
    )
  )
```