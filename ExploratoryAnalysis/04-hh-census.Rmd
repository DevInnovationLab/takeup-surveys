...
(Table shows share of households whose primary WP is non-DSW/ILC, but other WP has DSW/ILC)
...
```{r}
wp_other_table <- hh_census %>%
  mutate(
    used_other = wp_other_dsw == 1 | wp_other_ilc == 1
  ) %>%
  count(used_other) %>%
  mutate(
    Category = if_else(used_other, "DSW/ILC", "Non-DSW/ILC"),
    Percentage = round(n / sum(n) * 100, 1)
  ) %>%
  select(Category, Households = n, Percentage)

kable(wp_other_table,
      caption = "Number and Share of Households using WP without DSW/ILC whose other WP has DSW/ILC",
