...
# Number of reporting households
...
wtmt_used_survey %>%
  ...
  labs(
    ...
    caption = paste0(
      "Note: N=",
      n_households_survey,
      " households. Each household may report more than one \n",
      " treatment method. Households who do nothing counted from\n",
      "'do_anything == 0'"
    )
  ) +
  theme
...
ggplot(plot_data, aes(x = method_label, y = pct, fill = dataset)) +
  ...
  labs(
    ...
    y = "Share of Households (%)",
    caption = paste0(
      "Note: N=", n_households_survey, " for HH survey, N=", nrow(dplyr::filter(hh_census, rowSums(!is.na(dplyr::select(hh_census, dplyr::all_of(wtmt_cols)))) > 0))," for HH census.\n", 
...
ggplot(wtmt_used, aes(x = reorder(method_label, pct), y = pct)) +
  ...
  labs(
    ...
    y = "Share of Households (%)",
    caption = paste0(
      "Note: Household sample sizes – ", caption_text, 
...
# Define a function to generate the summary data by country
summarise_by_group <- function(df, country_name) {
  all_households <- df %>%
    summarise(
      group = "All Households",
      N = n(),
  ...
  bind_rows(all_households, chlorine_users, responders) %>%
    pivot_longer(cols = c(`Child under 5`, `Child under 2`, `Pregnant woman`),
                 names_to = "Characteristic",
                 values_to = "Percent") %>%
    mutate(
      group = factor(group, levels = c("All Households", "No DSW/ILC", "Applies Chlorine at Home")),
      country = country_name
    ) %>%
    mutate(N_label = paste0(group, ": ", N))
}

# Subset for Uganda and Malawi
uganda_data <- hh_census %>% filter(country == "UGANDA")
malawi_data <- hh_census %>% filter(country == "MALAWI")

# Apply summarisation
summary_uganda <- summarise_by_group(uganda_data, "UGANDA")
summary_malawi <- summarise_by_group(malawi_data, "MALAWI")

# Combine for plotting
comp_data <- bind_rows(summary_uganda, summary_malawi)

# Plotting function
plot_by_country <- function(df, country_name) {
  ggplot(df %>% filter(country == country_name),
         aes(x = Characteristic, y = Percent, fill = group)) +
    geom_col(position = position_dodge(width = 0.7), width = 0.6) +
    labs(
      title = paste("Household Composition in", country_name),
      x = "", y = "Share of Households (%)",
      fill = "Household Type"
    ) +
    scale_fill_viridis_d(option = "D", direction = -1) +
    theme_minimal()
...
# Create two plots
plot_ug <- plot_by_country(comp_data, "Uganda")
plot_mw <- plot_by_country(comp_data, "Malawi")

# Caption with N values by country
caption_text <- paste0(
  "Note:\n",
  "Uganda – ",
  paste(unique(summary_uganda$N_label), collapse = "; "), ".\n",
  "Malawi – ",
  paste(unique(summary_malawi$N_label), collapse = "; "), "."
)
