...
```{r}
hh_census %>% 
  mutate(using = (wp_intervention_type != "Non-program")) %>%
  group_by(village_id, country, sample_group) %>%
  summarise(using = sum(using, na.rm = TRUE)) %>%
  group_by(country, sample_group) %>%
  summarise(
    N = n(),
    se = sd(using, na.rm = TRUE)/N,
    n = mean(using)
  )
```
...
country <-
  ...
  summarise(
    ...
    `Households found` = n_distinct(household_id),
    `Response rate (%)` = mean(response),
    `Households served` = sum(wp_intervention_type != "Non-program", na.rm = TRUE),
    `Percent served (%)` = mean(wp_intervention_type != "Non-program", na.rm = TRUE)
  ) %>%
  mutate(sample_group = country)
```
...
hhc_sample <- 
  ...
  bind_rows(
    strata %>% dplyr::filter(country == "Malawi")
  ) %>%
  bind_rows(
    country %>% dplyr::filter(country == "Uganda")
  ) %>%
  bind_rows(
    strata %>% dplyr::filter(country == "Uganda")
  ) %>%
  ungroup %>%
  select(sample_group, everything()) %>%
  mutate(
    `Response rate (%)` = `Response rate (%)` %>% round(1),
    `Percent served (%)` = `Percent served (%)` %>% round(3) * 100
  ) %>%
  rename(` ` = sample_group) %>% 
  select(-country) %>%
  kable(
    format = "latex",
    caption = "Household census sample"
  )
...
map(
  c("Malawi", "Uganda"),
  ~ feols(
    hh_members ~ user | sample_group,
    cluster = ~village_id,
    data = hh_census %>%
      dplyr::filter(country == .x) %>%
      mutate(user = wp_intervention_type != "Non-program")
  ) %>%
    tidy
) %>% bind_rows %>%
...
villages <-
  ...
  mutate(
    evac = wp_intervention_type %in% c("DSW", "ILC water point", "ILC water collection point")
  ) %>%
  group_by(village_id, country, sample_group) %>%
  summarise(
    hhs = n(),
    hh_members = mean(hh_members, na.rm = TRUE),
    evac_hhs = sum(evac, na.rm = TRUE),
    evac_pct = evac_hhs/hhs
  ) 
  ...
  summarise(
    ...
    across(
      ...
      list(
        ...
        var = ~ var(., na.rm = TRUE),
        se = ~ sd(., na.rm = TRUE)/villages
      )
    )
  ) %>%
  mutate(
    hhs_ci = paste0("(", round(hhs_mean - 1.96 * hhs_se, 1), ", ", round(hhs_mean + 1.96 * hhs_se, 1), ")"), 
    evac_ci = paste0("(", round(evac_hhs_mean - 1.96 * evac_hhs_se, 1), ", ", round(evac_hhs_mean + 1.96 * evac_hhs_se, 1), ")")
  )

strata %>%
  select(-ends_with(c("_se", "_var"))) %>%
  kable(
    digits = 1,
...
strata <-
  ...
  mutate(
    f = villages/pop,
    total_stratum = evac_hhs_mean * pop,
    var_stratum = (pop^2) * (1 - f) * (evac_hhs_var / villages),
...
stratum <-
  strata %>%
  transmute(
    country, sample_group, pop, villages, members_av = hh_members,
    hhs = hhs_mean,
    hhs_ci = paste0("(", round(hhs - 1.96 * hhs_se, 1), ", ", round(hhs + 1.96 * hhs_se, 1), ")"),
    served_hhs_av = evac_hhs_mean, 
    served_hhs_se = evac_hhs_se, 
    served_hhs = total_stratum, 
    served_hhs_ci = paste0("(", round(served_hhs - 1.96 * se_stratum, 1), ", ", round(served_hhs + 1.96 * se_stratum, 1), ")"),
    members_av
  )
...
country <-
  ...
  summarise(
    ...
    pop = sum(pop),
    served_hhs = sum(total_stratum),
    var = sum(var_stratum),
    se = sqrt(var),
    members_av = mean(hh_members),
    ci_low = served_hhs - 1.96 * se,
    ci_high = served_hhs + 1.96 * se
  ) %>%
  transmute(
    country, villages,
    sample_group = country, 
    served_hhs, 
    pop, members_av,
    served_hhs_ci = paste0("(", ci_low %>% round(1), ", ", ci_high %>% round(1), ")"),
    members_av
  )
...
table <-
  ...
  mutate(served_people = served_hhs * members_av) %>%
  select(
    country, sample_group, pop, villages, hhs, served_hhs, served_hhs_ci, members_av, served
  )

table %>%
  kable(
    format = "latex",
    caption = "Population served by Evidence Action"
  )
```
...
# households using water points outside village
```{r}
hh_census %>%
  # dplyr::filter(intervention_type != "NON-PROGRAM") %>%
  dplyr::filter(!is.na(country)) %>%
  mutate(
    hh_location = case_when(
      inclusion == "Within village boundaries" ~ "Inside village",
      inclusion == "Up to 200m outside of village boundaries" ~ "Outside village (200m buffer)",
      TRUE ~ NA_character_
    )
  ) %>%
  dplyr::filter(!is.na(hh_location), !is.na(wp_source)) %>%
  group_by(country, hh_location, wp_source) %>%
  summarise(n = n(), .groups = "drop") %>%
  group_by(country, hh_location) %>%
  mutate(
    total = sum(n),
...
```{r}
wtmt_asked <- c(
  "wtmt_none", "wtmt_pretreated","wtmt_boil", "wtmt_dispcl", "wtmt_othercl", "wtmt_strain",
  "wtmt_filter", "wtmt_sand", "wtmt_solar", "wtmt_decant", "wtmt_other"
)
...
hh_census_valid <- hh_census[rowSums(!is.na(hh_census[, wtmt_asked])) > 0, ]
```

```{r}
summary_data <- hh_census_valid %>%
  st_drop_geometry() %>%
  mutate(
...
Highest in ILC-only villages
```{r}
summary_data <- hh_census_valid %>%
  st_drop_geometry() %>%
  mutate(
    used_any_treatment = rowSums(across(all_of(wtmt_used), ~ . == 1), na.rm = TRUE) > 0,
    category = case_when(
  ...
  summarise(
    total_n = n(),
    used = sum(used_any_treatment == TRUE, na.rm = TRUE),
    pct = round(100 * used / total_n, 1)
  ) %>%
  mutate(
...
ggplot(summary_data, aes(x = label, y = pct, fill = label)) +
  ...
  labs(
    title = "Share of Households Reporting Water Treatment by Village Program",
    x = NULL,
    y = "Share of All Households (%)",
    caption = paste0("Note: Includes households (n = ", sum(summary_data$total_n), 
                     "), that were asked water treatment qns in HH census (because their primary WP is not ILC or DSW).")
  ) +
  theme_minimal() +
  theme(
...
## Share of households using chlorine treatment by whether village has DSW/ILC
Higher in ILC than DSW and DSW-and-ILC. This is driven by wtmt_othercl, since wtmt_dispcl should be 0 (it's 1%) for ILC-only villages.
```{r}
summary_data <- hh_valid %>%
  ...
  summarise(
    total_n = n(),
    used_cl = sum(wtmt_anycl == TRUE, na.rm = TRUE),
