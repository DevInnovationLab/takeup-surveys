  ...
  map(
    ~ village_mean_ci(
      data = .x %>% 
        mutate(
          across(
            starts_with("disctcr_"),
            ~ case_when(disctcr_color == "1 - Clear" ~ 0, TRUE ~ . * 1)
          ),
          across(
            starts_with("discfcr_"),
            ~ case_when(discfcr_color == "1 - Clear" ~ 0, TRUE ~ . * 1)
          ),
          across(
            starts_with("metertcr_"),
            ~ case_when(metertcr_color == "1 - Clear" ~ 0, TRUE ~ . * 1)
          ),
          across(
            starts_with("meterfcr_"),
            ~ case_when(meterfcr_color == "1 - Clear" ~ 0, TRUE ~ . * 1)
          ),
          across(
            contains("cr_0"),
            ~ as.logical(.)
...
treatment %>%
  ...
  facet_grid(sample_group ~ country)
```
# If no self-report, no chlorination
      ...
      data = .x %>% 
        mutate(
          across(
            contains("cr_0"),
            ~ case_when(
              wp_intervention_type == "DSW" & is.na(make_watersafe_2) ~ 0,
              wp_intervention_type == "DSW" & !make_watersafe_2 ~ 0,
              TRUE ~ . * 1
            ) %>% as.logical
          )
        ),
