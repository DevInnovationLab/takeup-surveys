...
test_comparison_malawi <- hh_survey_spotchecks_malawi %>%
  ...
  dplyr::filter(abs(fcr) < 100)%>% # to get rid of one really high result, clearly an error
  dplyr::filter(abs(tcr) < 100) %>%  
  summarize(
...
test_comparison_uganda <- hh_survey_spotchecks_uganda %>%
  ...
  dplyr::filter(fcr < 100) %>% # to get rid of one really high result, clearly an error
  summarize(
    diff_tcr_mean = mean(diff_tcr, na.rm = TRUE),
    diff_fcr_mean = mean(diff_fcr, na.rm = TRUE),
    n = n()
...
"comment" to see if there's anything interesting

for hh survey:
- (low priority) hhs_section_1 to hhs_section_10

for hh census (low priority):
- hhc_section_1
- hhc_section_2
- hhc_section_4
- hhc_section_3

for promoter survey (low priority): 
- prom_section_1
- prom_section_2
- prom_section_3
- prom_section_4
...
currently - not much important to compare, because we don't have chlorine tests

1) confirmation of correct WP
<b>1.9 Is ${wp_name_pl} the primary water source that your household uses for collecting drinking water?: only 1 case of no (across both countries) - good sign
