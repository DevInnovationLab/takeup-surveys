...
uganda_model <- lm(disctcr_02 ~ promoter_list, data = uganda_promoter_wp)
malawi_model <- lm(disctcr_02 ~ promoter_list, data = malawi_promoter_wp)
...
uganda_model <- lm(disctcr_02 ~ survey, data = uganda_promoter_wp)
malawi_model <- lm(disctcr_02 ~ survey, data = malawi_promoter_wp)

stargazer(uganda_model, malawi_model,
          type = "text",
          title = "Association between monitoring survey and Chlorination",
          dep.var.labels = "disctcr_02",
          covariate.labels = c("Listed by Promoter"),
          column.labels = c("Uganda", "Malawi"),
          model.numbers = FALSE,
...
```{r}
promoter_wp_households_sensitivity <- hh_survey %>%
  dplyr::filter(promoter_surveyed == 1) %>%
  mutate(
    rank = as.numeric(rank),
    survey = as.character(survey),  # Convert to character first
...
promoter_wp_households_sensitivity
```
