...
This is the code to generate the estimates tables using the monitoring (water-point level) approach, as well as tables and statistics used in sections 6 and 7 of the takeup report.

# notes
- I have used pm_survey to count the number of households in the promoter survey in table 11
- I have used ea_list_uganda and ea_list_malawi for the number of WPs int the EA list in table 9
- I have used pm_survey_households for table 11 and to tackle double-counted hhs in table 14
- For table 1 ex households located outside of village, I did a rough calculation based on finding the share of households for each sample group that was outside the village, and then lowering the total for that sample group by this share. Ideally, I just rerun the table 1 code here but without households that have inclusion as within 200m of village boundary for a more exact number

to do
- move pm_survey_households to wrangling - but this does not influence table 8 estimates
...
## load rank dataset (from randomization)
```{r}
pm_survey_clean <-
  read_rds(
    file.path(
...
### account for HHs that should have been in the monitoring survey
```{r}
# if this code is NOT commented out, the tables below will include households that would have been selected for the monitoring survey but had already been present in the household survey
# hh_survey <- hh_survey %>%
#   mutate(
#     rank = as.numeric(rank),
#     survey = as.character(survey),  # Convert to character first
#     survey = ifelse(survey == "Household Survey" & !is.na(rank) & rank <= 4,
#                     "Monitoring Survey",
#                     survey)
#   )
```

## pm_survey_households - cleaning, to be moved to wrangling
```{r}
# delete test village
pm_survey_households <- pm_survey_households %>%
  dplyr::filter(village_id != "1010602004")

# identify water points that don't match to wp census
unmatched_wps <- pm_survey_households %>%
  dplyr::filter(!wp_id_c %in% wp_census$wp_id) %>%
  select(wp_id_c, wp_name, village_id, country) %>%
  distinct()

# drop replaced villages
pm_survey_households <- pm_survey_households %>%
  dplyr::filter(village_id != "2030413108")

unmatched_wps

# manually recode unmatched water point IDs
wp_id_recoding <- tribble(
  ~wp_id_c_old,          ~wp_id_c_new,
  "2070301002007",       "2070302012007",
  "2070301002004",       "2070302012004"
)

# apply recoding to pm_survey_households
pm_survey_households <- pm_survey_households %>%
  left_join(wp_id_recoding, by = c("wp_id_c" = "wp_id_c_old")) %>%
  mutate(wp_id_c = coalesce(wp_id_c_new, wp_id_c)) %>%
  select(-wp_id_c_new)

# check if any are still unmatched
unmatched_wps_after <- pm_survey_households %>%
  dplyr::filter(!wp_id_c %in% wp_census$wp_id) %>%
  select(wp_id_c, wp_name) %>%
  distinct()

unmatched_wps_after

# drop remaining for now because these are Uganda ILC - hence why they don't match
pm_survey_households <- pm_survey_households %>%
  dplyr::filter(!wp_id_c %in% wp_census$wp_id)
```

## country totals
classifying by the sample group of the district
```{r}
country_totals <- tibble::tribble(
  ~country, ~intervention, ~sample_group, ~country_total,
  "Uganda", "DSW", "Footprint", 5456,
  "Uganda", "DSW", "Expansion", 12262,
  "Malawi", "DSW", "Footprint", 3844,
  "Malawi", "DSW", "Expansion", 11292,
  "Malawi", "DSW", "ILC", 1062, 
  "Malawi", "ILC", "ILC", 1720 # 2024 times 0.85
)
```

below, classifying by the sample group of the water point -  old version
```{r}
# malawi 
...
#   # in the row below, change to sample_group to use village classification instead
...
### exploratory: looking into whether classifying as expansion/footprint by water point vs village makes a difference
```{r}
# # malawi DSW classification table
# malawi_dsw_classification <- malawi_wp %>%
#   dplyr::filter(intervention == "DSW") %>%
#   count(wp_sample_group, sample_group) %>%
#   pivot_wider(names_from = sample_group, values_from = n, values_fill = 0)
# 
# print("Malawi DSW Water Points: WP Classification (rows) vs Village Classification (cols)")
# print(malawi_dsw_classification)
# 
# # uganda DSW classification table
# uganda_dsw_classification <- uganda_wp %>%
#   dplyr::filter(intervention == "DSW") %>%
#   count(wp_sample_group, sample_group) %>%
#   pivot_wider(names_from = sample_group, values_from = n, values_fill = 0)
# 
# print("Uganda DSW Water Points: WP Classification (rows) vs Village Classification (cols)")
# print(uganda_dsw_classification)
```
...
compute_se <- function(values, ses = NULL, include_within = FALSE) {
  ...
  # within water point variance (this will apply for chlorination and number of hh members)
  within_var <- 0
  if (include_within && K > 0 && length(ses) == K) {
    within_var <- sum(ses^2, na.rm = TRUE) / (K^2)
  }
  # total variance is a sum of between and within
...
create_summary_table <- function(count_tbl, stats_tbl, totals_tbl, 
  ...
  count_tbl %>%
    ...
    mutate(
      ...
      ci_lower_chl_rate_tcr = avg_chl_rate_tcr - 1.96 * se_chl_rate_tcr,  
      ci_upper_chl_rate_tcr = avg_chl_rate_tcr + 1.96 * se_chl_rate_tcr,  
      ci_lower_chl_rate_fcr = avg_chl_rate_fcr - 1.96 * se_chl_rate_fcr,  
      ci_upper_chl_rate_fcr = avg_chl_rate_fcr + 1.96 * se_chl_rate_fcr 
    ) %>%
...
## create function to generate latex table from summary table and country totals
```{r}
generate_latex_table <- function(summary_table, country_totals_table, 
  ...
  table_data <- summary_table %>%
    mutate(
      sample_group = case_when(
        sample_group == "Footprint" & intervention == "DSW" ~ "Footprint",
        sample_group == "Expansion" & intervention == "DSW" ~ "Expansion",
        sample_group == "ILC" & intervention == "DSW" ~ "DSW in ILC vill.",
        sample_group == "ILC" & intervention == "ILC" ~ "ILC",
        TRUE ~ sample_group
      ),
    ...
    dplyr::filter(intervention == "DSW" | (sample_group == "ILC" & intervention == "ILC")) %>%
    dplyr::filter(!(country == "Uganda" & intervention == "ILC")) %>%
    arrange(country, order_var)
  
  # separate by country
  malawi_data <- table_data %>%
    dplyr::filter(country == "Malawi") %>%
    select(sample_group, avg_hh_count, se_hh_count, avg_hh_size, se_hh_size, 
           indiv_access_sample, se_indiv_access_sample, country_total, 
           indiv_access_popn, ci_lower_indiv_access_popn, ci_upper_indiv_access_popn,
           avg_chl_rate_tcr, ci_lower_chl_rate_tcr, ci_upper_chl_rate_tcr,
           avg_chl_rate_fcr, ci_lower_chl_rate_fcr, ci_upper_chl_rate_fcr)
  
  uganda_data <- table_data %>%
  ...
  # get country totals
  malawi_total <- country_totals_table %>%
    dplyr::filter(country == "Malawi")
  
  uganda_total <- country_totals_table %>%
    dplyr::filter(country == "Uganda")
  ...
  cat("\\hline \\hline\n")
  cat("& (1) & (2) & (3) & (4) & (5) & (6) & (7)\\\\\n")
  cat("& \\multicolumn{3}{c}{\\textbf{Sample}} & \\multicolumn{2}{c}{\\textbf{Country-level estimates}} & & \\\\\n")
  cat("\\cmidrule(lr){2-4} \\cmidrule(lr){5-6}\n")
  cat(" & \\shortstack{Avg \\# \\\\of HH \\\\ (SE)} &\\shortstack{Avg \\# of \\\\ Members\\\\ (SE)} & \\shortstack{\\# people \\\\ per WP\\\\ (SE)} & \\shortstack{Country \\\\\\#WP} & \\shortstack{People using\\\\ DSW/ILC \\\\ (95\\% CI)} & \\shortstack{TCR>0.2ppm \\\\ (95\\% CI)} & \\shortstack{FCR>0.2ppm \\\\ (95\\% CI)}\\\\\n")
  cat("\\hline\n")
  cat("\\textbf{Malawi} & & & & & & & \\\\\n")
  cat("\\hline\n")
  ...
  for(i in 1:nrow(malawi_data)) {
    row <- malawi_data[i,]
    cat(sprintf("%s & \\shortstack{%.2f \\\\ (%.2f)} & \\shortstack{%.2f \\\\ (%.2f)} & \\shortstack{%.2f \\\\ (%.2f)} & %s & \\shortstack{%s \\\\ (%s, %s)} & \\shortstack{%.1f\\%% \\\\ (%.1f\\%%, %.1f\\%%)} & \\shortstack{%.1f\\%% \\\\ (%.1f\\%%, %.1f\\%%)}\\\\\n",
                row$sample_group,
  ...
  cat("\\hline\\hline\n")
  cat("\\multicolumn{8}{p{\\linewidth}}{\\footnotesize Notes: In col. 3, the number of people per water point is calculated as the product of the average number of households per water point and the average number of members per household. The population estimate of people using water points with DSW or ILC in col. 5 is calculated by multiplying the number of people per water point by the number of water points of that type in the country. Col. 6 reports the share of households with positive chlorine among those using DSW or ILC as their primary water source. In this table, positive chlorine is defined as the total chlorination level higher than 0.2 ppm measured with color-wheel.}\\\\\n")
  cat("\\end{tabular}\n")
  cat("\\end{table}\n")
}
```
...
wp_table <- wp_census %>%
  ...
  left_join(country_totals, by = c("country", "sample_group", "intervention")) %>%
  left_join(
    ea_list_malawi %>%
      group_by(country, sample_group, intervention) %>%
...
wp_table_latex <- wp_table %>%
  ...
  select(country, sample_group, intervention, country_total, wp_census, wp_functional, ea_list_count, promoter_surveyed)

malawi_footprint <- wp_table_latex %>%
  dplyr::filter(country == "Malawi", sample_group == "Footprint", intervention == "DSW") %>%
  mutate(sample_group = "Footprint")

malawi_expansion <- wp_table_latex %>%
  dplyr::filter(country == "Malawi", sample_group == "Expansion", intervention == "DSW") %>%
  mutate(sample_group = "Expansion")

malawi_dsw_ilc <- wp_table_latex %>%
  dplyr::filter(country == "Malawi", sample_group == "ILC", intervention == "DSW") %>%
  mutate(sample_group = "DSW in ILC vill.")

malawi_ilc <- wp_table_latex %>%
  dplyr::filter(country == "Malawi", sample_group == "ILC", intervention == "ILC") %>%
  mutate(sample_group = "ILC")

malawi_data <- bind_rows(malawi_footprint, malawi_expansion, malawi_dsw_ilc, malawi_ilc) %>%
  select(-country, -intervention)

uganda_footprint <- wp_table_latex %>%
  dplyr::filter(country == "Uganda", sample_group == "Footprint", intervention == "DSW") %>%
  mutate(sample_group = "Footprint")

uganda_expansion <- wp_table_latex %>%
  dplyr::filter(country == "Uganda", sample_group == "Expansion", intervention == "DSW") %>%
  mutate(sample_group = "Expansion")
...
cat("\\hline\\hline\n")
cat("& (1) & (2) & (3) & (4) & (5)\\\\\n")
cat("                   & Country Total & WP Census & \\shortstack{WP Census \\\\Functional} & EA Promoter List & Promoter Survey\\\\\n")
cat("\\hline\n")
cat("\\textbf{Malawi}                  &            &           &        &        &         \\\\\n")
cat("\\hline\n")
...
for(i in 1:nrow(malawi_section)) {
  ...
  cat(sprintf("%s & %s & %s & %s & %s & %s \\\\\n",
              row$sample_group, row$country_total, row$wp_census, 
...
### text for table caption
For Uganda, 2 of the 112 water points in the EA promoter list were identified as ILC, and are not shown. For Malawi, 2 out of 137 were identified as Non-program and are not shown.
```{r}
ea_list_malawi
...
## households (11)
```{r}
# renaming intervention in hh census for consistency - this is needed if intervention in hh_census is called wp_intervention
hh_census <- hh_census %>%
...
pm_users <- wp_census %>%
  mutate(pm_users = as.numeric(pm_users)) %>%
  group_by(country, sample_group, intervention) %>% 
  summarise(
    pm_users = sum(pm_users, na.rm = TRUE),               
    .groups = "drop"
  )
...
pm_users_census <- wp_census %>%
  mutate(pm_users_census = as.numeric(pm_users_census)) %>%
  group_by(country, sample_group, intervention) %>% 
  summarise(
    pm_users_census = sum(pm_users_census, na.rm = TRUE),               
    .groups = "drop"
  )

# number of households whose self-report in HH census matches the promoter's report
pm_hh_matched <- pm_survey_households %>%
  left_join(wp_census %>% select(wp_id, sample_group, intervention), 
            by = c("wp_id_c" = "wp_id")) %>%
...
pm_users_outside <- wp_census %>%
  mutate(pm_users_outside = as.numeric(pm_users_outside)) %>%
  group_by(country, sample_group, intervention) %>% 
  summarise(
    pm_users_outside = sum(pm_users_outside, na.rm = TRUE),               
    .groups = "drop"
  )
...
pm_avg_hh_per_wp <- wp_census %>%
  mutate(pm_users = as.numeric(pm_users)) %>%
  group_by(country, sample_group, intervention) %>% 
  summarise(
    pm_avg_hh_per_wp = round(mean(pm_users, na.rm = TRUE), 2),               
    .groups = "drop"
  )
...
# table
hh_tbl <- wp_count_pm  %>%
  left_join(pm_users, by = c("country", "sample_group", "intervention")) %>%
  left_join(pm_users_outside, by = c("country", "sample_group", "intervention")) %>%
  left_join(pm_users_census, by = c("country", "sample_group", "intervention")) %>%
  left_join(pm_hh_matched, by = c("country", "sample_group", "intervention")) %>%
  left_join(pm_avg_hh_per_wp, by = c("country", "sample_group", "intervention")) %>%
  left_join(wp_count_wp_census, by = c("country", "sample_group", "intervention")) %>%
  left_join(census_hh_listed, by = c("country", "sample_group", "intervention")) %>%
  left_join(census_avg_hh_per_wp, by = c("country", "sample_group", "intervention"))

hh_tbl

# creating latex table
# prepare data for table
hh_table_latex <- hh_tbl %>%
  mutate(
    sample_group = case_when(
      sample_group == "Footprint" & intervention == "DSW" ~ "Footprint",
      sample_group == "Expansion" & intervention == "DSW" ~ "Expansion",
...
# create separate datasets for Malawi and Uganda
malawi_data <- hh_table_latex %>%
  dplyr::filter(country == "Malawi") %>%
  select(sample_group, wp_count_pm, pm_users, pm_users_outside, pm_users_census, 
         pm_hh_matched, pm_avg_hh_per_wp, wp_count_wp_census, census_hh_listed, census_avg_hh_per_wp)

uganda_data <- hh_table_latex %>%
  dplyr::filter(country == "Uganda") %>%
  select(sample_group, wp_count_pm, pm_users, pm_users_outside, pm_users_census, 
         pm_hh_matched, pm_avg_hh_per_wp, wp_count_wp_census, census_hh_listed, census_avg_hh_per_wp)
...
malawi_total <- malawi_data %>%
  summarise(
    sample_group = "\\textbf{Total}",
    wp_count_pm = sum(wp_count_pm, na.rm = TRUE),
    pm_users = sum(pm_users, na.rm = TRUE),
    pm_users_outside = sum(pm_users_outside, na.rm = TRUE),
    pm_users_census = sum(pm_users_census, na.rm = TRUE),
    pm_hh_matched = sum(pm_hh_matched, na.rm = TRUE),
    pm_avg_hh_per_wp = NA,
    wp_count_wp_census = sum(wp_count_wp_census, na.rm = TRUE),
    census_hh_listed = sum(census_hh_listed, na.rm = TRUE),
    census_avg_hh_per_wp = NA
  )
...
overall_total <- bind_rows(malawi_data, uganda_data) %>%
  summarise(
    sample_group = "\\textbf{Overall Total}",
    wp_count_pm = sum(wp_count_pm, na.rm = TRUE),
    pm_users = sum(pm_users, na.rm = TRUE),
    pm_users_outside = sum(pm_users_outside, na.rm = TRUE),
    pm_users_census = sum(pm_users_census, na.rm = TRUE),
...
malawi_section <- malawi_section %>%
  mutate(across(-sample_group, ~format_num(.x)))
...
cat("\\begin{table}[htbp]\\centering\n")
cat("\\def\\sym#1{\\ifmmode^{#1}\\else\\(^{#1}\\)\\fi}\n")
cat("\\caption{Number of households identified by different instruments \\label{hh_identified}}\n")
cat("\\begin{tabular}{l*{9}{c}}\n")
cat("\\hline \\hline\n")
cat("& (1) & (2) & (3) & (4) & (5) & (6) & (7) & (8) & (9)\\\\\n")
cat("& \\multicolumn{6}{c}{\\textbf{Promoter Survey}} & \\multicolumn{3}{c}{{\\textbf{WP \\& HH Census}}} \\\\\n")
cat("\\cmidrule(lr){2-7} \\cmidrule(lr){8-10}\n")
cat(" & \\shortstack{\\#WP} & \\shortstack{Total \\\\\\#HH\\\\ Listed} & \\shortstack{\\#HH \\\\ Outside \\\\ Village} & \\shortstack{\\#HH \\\\ in Census} & \\shortstack{\\#HH \\\\ Matched} & \\shortstack{Avg \\#HH \\\\ per WP } & \\shortstack{\\#WP} & \\shortstack{Total \\\\ \\#HH} & \\shortstack{Avg \\#HH \\\\ per WP}\\\\\n")
cat("\\hline\n")
cat("\\textbf{Malawi}                  &            &            &          & & & & & &  \\\\\n")
cat("\\hline\n")
...
for(i in 1:nrow(malawi_section)) {
  ...
  cat(sprintf("%s & %s & %s & %s & %s & %s & %s & %s & %s & %s\\\\\n",
              row$sample_group, row$wp_count_pm, row$pm_users, row$pm_users_outside,
              row$pm_users_census, row$pm_hh_matched, row$pm_avg_hh_per_wp,
              row$wp_count_wp_census, row$census_hh_listed, row$census_avg_hh_per_wp))
}
...
cat("\\hline\n")
cat(sprintf("\\textbf{Overall Total} & %s & %s & %s & %s & %s & %s & %s & %s & %s\\\\\n",
            overall_total$wp_count_pm, overall_total$pm_users, overall_total$pm_users_outside,
            overall_total$pm_users_census, overall_total$pm_hh_matched, overall_total$pm_avg_hh_per_wp,
            overall_total$wp_count_wp_census, overall_total$census_hh_listed, overall_total$census_avg_hh_per_wp))
cat("\\hline\\hline\n")
cat("\\multicolumn{10}{p{\\linewidth}}{\\footnotesize Notes: Total \\#HH Listed refers to the total number of user households listed by promoters for water points of given type. This double-counts some households, as they are listed as users by several promoters. \\#HH Outside Village shows the total number of households that promoters say are users located in a different village than the water point. \\#HH in Census is the total of the number of households listed for each water point that were also identified in the household census. \\#HH Matched is a subset of this, only containing households where the household's report of its primary water point matches the promoter report.}\\\\\n")
cat("\\end{tabular}\n")
cat("\\end{table}\n")
```

# final table 8
- should increase the number of users by 1 / response rate
- 
...
hh_count <- wp_census %>%
  ...
  dplyr::filter(intervention %in% c("DSW", "ILC")) %>%
  # use pm list of users (a variable originally from the promoter survey)
...
# chlorination and number of household members
hh_stats <- hh_survey %>%
  ...
  summarize(
    ...
    chl_rate_tcr = mean(disctcr_02, na.rm = TRUE),
    chl_rate_fcr = mean(discfcr_02, na.rm = TRUE),
    se_chl_rate_tcr = sqrt(chl_rate_tcr * (1 - chl_rate_tcr) / n()),  # within-WP SE
    se_chl_rate_fcr = sqrt(chl_rate_fcr * (1 - chl_rate_fcr) / n()),
    .groups = "drop_last"
  ) %>%
  group_by(country, sample_group, intervention) %>%
  summarize(
    wp_hh_sizes = list(hh_size[!is.na(hh_size)]),
    wp_rates_tcr = list(chl_rate_tcr[!is.na(chl_rate_tcr)]),
    wp_rates_fcr = list(chl_rate_fcr[!is.na(chl_rate_fcr)]),
    wp_ses_tcr = list(se_chl_rate_tcr[!is.na(se_chl_rate_tcr)]),  # RENAME from wp_ses
    wp_ses_fcr = list(se_chl_rate_fcr[!is.na(se_chl_rate_fcr)]),
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(
    hh_tmp = list(compute_se(wp_hh_sizes, include_within = FALSE)),
    chl_tmp_tcr = list(compute_se(wp_rates_tcr, wp_ses_tcr, include_within = TRUE)),  
    chl_tmp_fcr = list(compute_se(wp_rates_fcr, wp_ses_fcr, include_within = TRUE))  
    ) %>%
  unnest_wider(hh_tmp, names_sep = "_") %>%
  unnest_wider(chl_tmp_tcr, names_sep = "_") %>%
  unnest_wider(chl_tmp_fcr, names_sep = "_") %>%
  rename(
    avg_hh_size = hh_tmp_mean_val,
    se_hh_size = hh_tmp_se,
    avg_chl_rate_tcr = chl_tmp_tcr_mean_val,  
    se_chl_rate_tcr = chl_tmp_tcr_se,         
    avg_chl_rate_fcr = chl_tmp_fcr_mean_val, 
    se_chl_rate_fcr = chl_tmp_fcr_se         
  ) %>%
  select(country, sample_group, intervention, avg_hh_size, se_hh_size, 
         avg_chl_rate_tcr, se_chl_rate_tcr, avg_chl_rate_fcr, se_chl_rate_fcr)  
...
generate_latex_table(
  summary_table = table8,
  country_totals_table = country_estimates_table8,
  caption = "User estimates for water points in the promoter survey using water-point level data from the promoter survey and monitoring survey",
  label = "beneficiaries_pm"
)
```
...
# calculate household size from hh_census at the water point level
hh_size_from_census <- hh_census %>%
  # use water points in promoter survey
  dplyr::filter(promoter_surveyed == 1) %>%
  group_by(country, sample_group, intervention, wp_id) %>%
  summarize(
...
# calculate chlorination from hh_survey and join with hh_size
hh_stats <- hh_survey %>%
  ...
  summarize(
    chl_rate_tcr = mean(disctcr_02, na.rm = TRUE),
    chl_rate_fcr = mean(discfcr_02, na.rm = TRUE),
    se_chl_rate_tcr = sqrt(chl_rate_tcr * (1 - chl_rate_tcr) / n()),
    se_chl_rate_fcr = sqrt(chl_rate_fcr * (1 - chl_rate_fcr) / n()),
    .groups = "drop"
  ) %>%
  # join household size from census
  left_join(hh_size_from_census, by = c("country", "sample_group", "intervention", "wp_id")) %>%
  group_by(country, sample_group, intervention) %>%
  summarize(
    wp_hh_sizes = list(hh_size[!is.na(hh_size)]),
    wp_rates_tcr = list(chl_rate_tcr[!is.na(chl_rate_tcr)]),
    wp_rates_fcr = list(chl_rate_fcr[!is.na(chl_rate_fcr)]),
    wp_ses_tcr = list(se_chl_rate_tcr[!is.na(se_chl_rate_tcr)]),
    wp_ses_fcr = list(se_chl_rate_fcr[!is.na(se_chl_rate_fcr)]),
    .groups = "drop"
  ) %>%
  rowwise() %>%
...
# generate latex table
generate_latex_table(
  summary_table = table10,
  country_totals_table = country_estimates_table10,
  caption = "User estimates for water points in the promoter survey using water-point level data from the household census and household survey",
  label = "beneficiaries_hh"
)
```
...
```{r}
hh_count <- hh_census %>%
  # use household census and do not filter for water point being in promoter survey 
  group_by(country, sample_group, intervention, wp_id) %>%
  summarise(
...
# generate latex table
generate_latex_table(
  summary_table = table12,
  country_totals_table = country_estimates_table12,
  caption = "User estimates for water points in the water point census using water-point level data from the household census and household survey",
  label = "beneficiaries_wp_census"
)
```
...
```{r}
# calculate total users per water point by summing hh_members
users_per_wp <- hh_census_imputed %>%
  group_by(country, sample_group, intervention, wp_id) %>%
  summarise(
    total_users = sum(hh_members, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  group_by(country, sample_group, intervention) %>%
  summarise(
    avg_users_per_wp = mean(total_users, na.rm = TRUE),
    se_users_per_wp = sd(total_users, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
  ) %>%
  dplyr::filter(intervention %in% c("DSW", "ILC"))

# calculate chlorination from hh_survey
hh_stats_revised <- hh_survey %>%
  dplyr::filter(survey == "Household Survey") %>%
  group_by(country, sample_group, intervention, wp_id) %>%
  summarize(
    chl_rate_tcr = mean(disctcr_02, na.rm = TRUE),
...
# join the two tables
table12 <- users_per_wp %>%
  left_join(hh_stats_revised, by = c("country", "sample_group", "intervention")) %>%
  left_join(country_totals, by = c("country", "sample_group", "intervention")) %>%
  mutate(
    total_users = avg_users_per_wp * country_total,
    se_total_users = se_users_per_wp * country_total,
    total_beneficiaries_tcr = total_users * avg_chl_rate_tcr,
    se_total_beneficiaries_tcr = total_beneficiaries_tcr * sqrt((se_users_per_wp/avg_users_per_wp)^2 + (se_chl_rate_tcr/avg_chl_rate_tcr)^2),
    total_beneficiaries_fcr = total_users * avg_chl_rate_fcr,
    se_total_beneficiaries_fcr = total_beneficiaries_fcr * sqrt((se_users_per_wp/avg_users_per_wp)^2 + (se_chl_rate_fcr/avg_chl_rate_fcr)^2)
  )

table12 %>% select(country, sample_group, intervention, avg_chl_rate_tcr, se_chl_rate_tcr)

# create country estimates table directly
country_estimates_table12 <- table12 %>%
  group_by(country) %>%
  summarise(
    total_users = sum(total_users, na.rm = TRUE),
    se_total_users = sqrt(sum(se_total_users^2, na.rm = TRUE)),
    total_beneficiaries_tcr = sum(total_beneficiaries_tcr, na.rm = TRUE),
    se_total_beneficiaries_tcr = sqrt(sum(se_total_beneficiaries_tcr^2, na.rm = TRUE)),
    total_beneficiaries_fcr = sum(total_beneficiaries_fcr, na.rm = TRUE),
    se_total_beneficiaries_fcr = sqrt(sum(se_total_beneficiaries_fcr^2, na.rm = TRUE)),
    .groups = "drop"
  )
...
# average number of households inside village per water point
hh_stats <- hh_survey %>%
  # use monitoring survey
  dplyr::filter(survey == "Monitoring Survey") %>%
  group_by(country, sample_group, intervention, wp_id) %>%
  ...
  select(country, sample_group, intervention, avg_hh_size, se_hh_size, avg_chl_rate_tcr, se_chl_rate_tcr, avg_chl_rate_fcr, se_chl_rate_fcr)
...
# generate latex table
generate_latex_table(
  summary_table = table13,
  country_totals_table = country_estimates_table13,
  caption = "User estimates for water points in the promoter survey using water-point level data from the promoter survey and monitoring survey, excluding households outside of village",
  label = "beneficiaries_exout"
)
```


## excluding HHs outside village and double counted HHs (14)
exactly like table 8, but excluding households located outside village and getting rid of double-counting
```{r}
...
# for double-counted households, determine their "correct" water point assignment - based on self-report in hh_census, as long as this matches one of the water points that the household was listed for
...
hh_count <- wp_census %>%
  ...
  # subtract both households outside and households that were double-counted
    pm_users_adjusted = pm_users - pm_users_outside - households_to_subtract
  ) %>%
  group_by(country, sample_group, intervention) %>%
  summarise(
    wp_values = list(pm_users_adjusted[!is.na(pm_users_adjusted)]), 
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(tmp = list(compute_se(wp_values, include_within = FALSE))) %>%
...
# country estimates table
country_estimates_table14 <- compute_country_totals(table14)
country_estimates_table14

# generate latex table
generate_latex_table(
  summary_table = table14,
  country_totals_table = country_estimates_table14,
  caption = "User estimates for water points in the water point census using water-point level data from the promoter survey and monitoring survey, excluding households outside of village and double-counted households",
  label = "beneficiaries_exout_exdouble"
)
```

# waterfall charts
## share of households outside village - used to adjust table 1 estimates
```{r}
...
# country estimates for table 1
country_estimates_table1 <- table1 %>%
  group_by(country) %>%
  summarise(indiv_access_popn = sum(indiv_access_popn, na.rm = TRUE), .groups = "drop")

country_estimates_table1

country_estimates_table1_exout <- table1_exout %>%
  group_by(country) %>%
  summarise(indiv_access_popn = sum(indiv_access_popn, na.rm = TRUE), .groups = "drop")

country_estimates_table1_exout
```
...
estimates <- table1 %>%
  ...
  left_join(
    table12_exout %>% group_by(country) %>% summarise(table12_exout = sum(indiv_access_popn, na.rm = TRUE)),
    by = "country"
  ) %>%
  left_join(
...
create_waterfall <- function(country_name, country_data) {
  ...
  amount = c(
    ...
    country_data$table12_exout - country_data$table1_exout,
    country_data$table10_exout - country_data$table12_exout,
...
Again, ideally table 1 numbers will automatically come from table 1 calculations file - I need to add this

difference between table 1 and table 8 estimates is statistically significant for both countries
```{r}
# UGANDA
# t1 uganda
...
difference is not statistically significant for either country
```{r}
#t12 uganda
...
## comparing CIs for estimates that differ by "dataset" - table 10 ex outside hhs vs table 14
```{r}
# prepare data from both tables
table14_data <- country_estimates_table14 %>%
  mutate(table = "Promoter & monitoring \n survey data",
         estimate = total_indiv_access_popn,
         ci_lower = ci_lower_total_access,
         ci_upper = ci_upper_total_access)

table10_data <- country_estimates_table10_exout %>%
  mutate(table = "Household census & \n survey data",
         estimate = total_indiv_access_popn,
         ci_lower = ci_lower_total_access,
         ci_upper = ci_upper_total_access)

# combine the data
plot_data <- bind_rows(table14_data, table10_data) %>%
  dplyr::select(country, table, estimate, ci_lower, ci_upper)

# create the plot
ggplot(plot_data, aes(x = table, y = estimate, color = country)) +
  ...
  labs(title = "Comparison of Individuals with Access Estimates",
       subtitle = "95% Confidence Intervals shown; remaining difference is unexplained besides dataset: both \nestimates have same water point sample & exclude out-of-village and double-counted HHs",
       x = NULL,
       y = "Individuals with Access Estimates",
       color = "Country") +
  theme_minimal() +
...
balance_table16 <- wp_census %>%
  dplyr::filter(country == "Uganda", sample_group == "Footprint", intervention_type == "DSW") %>%
  # I treat NAs as false for the purpose of balance tests (if NA on whether a water point is in the EA list, I treat is as though it is FALSE)
  mutate(
...
significant difference in disp_filled and wp_func
```{r}
...
## Malawi ILC (22)
wp_func significantly different, but this isn't very relevant
...
uganda_hh_survey <- hh_survey %>%
  dplyr::filter(country == "Uganda") %>%
  # I treat NAs as false for the purpose of balance tests (if NA on whether a water point is in the EA list or promoter survey, I treat is as though it is FALSE)
  mutate(
...
model1 <- lm(disctcr_02 ~ promoter_surveyed, data = uganda_hh_survey)
# column 2: EA list, Uganda
model2 <- lm(disctcr_02 ~ pm_ea_list, data = uganda_hh_survey)
# column 3: both, Uganda
model3 <- lm(disctcr_02 ~ pm_ea_list + promoter_surveyed, data = uganda_hh_survey)
# column 4: promoter survey, Malawi
model4 <- lm(disctcr_02 ~ promoter_surveyed, data = malawi_hh_survey)
# column 5: EA list, malawi
model5 <- lm(disctcr_02 ~ pm_ea_list, data = malawi_hh_survey)
# column 6: both, Malawi
model6 <- lm(disctcr_02 ~ pm_ea_list + promoter_surveyed, data = malawi_hh_survey)

# view in R
stargazer(model1, model2, model3, model4, model5, model6,
          type = "text",
          title = "Association between Promoter Survey, EA List and Chlorination",
          dep.var.labels = "TCR >0.2 (Colorwheel)",
...
stargazer(model1, model2, model3, model4, model5, model6,
          ...
          dep.var.labels = "TCR >0.2ppm (Colorwheel)",
          covariate.labels = c("Promoter Survey", "EA list"),
...
## household selection
association is positive, but only significant if we regress disctcr. if the dependent variable is disctcr_02, then it is not
(I clustered SEs by water point)
```{r}
library(estimatr)
...
## sample: EA list vs promoter survey - uganda
```{r}
ea_promoters_uganda <-
  ea_promoters_uganda %>%
...
glue("In Malawi, Evidence Action provided information for {n_promoters} promoters in the sampled villages, representing {n_unique_wps} unique water points. Out of these, {n_in_wp_census} are in the water point census, and {n_promoter_surveyed} are included in the promoter survey. There are {n_surveyed_no_ea_info} promoter surveys for water points for which Evidence Action did not provide contact information (primarily ILC water collection points), for a total of {n_total_promoter_surveys} promoter surveys in Malawi.")
```
...
## share of hhs that appear more than once in promoter lists
```{r}
## share of matched hhs that appear more than once in promoter lists + that appear more than once despite self-reporting only using 1 water point (in past month)
hh_double_count_stats <- hh_census %>%
  # group_by(country) %>%
  dplyr::filter(!is.na(promoter_list)) %>%
  summarise(
    ...
    # wp_secweek is FALSE -> hh reports not using any additional water sources
    hh_double_counted = sum(promoter_list_mentions > 1 & wp_secweek == FALSE, na.rm = TRUE),
    share_double_counted = hh_double_counted / total_hh,
    share_double_counted_pct = round(share_double_counted * 100, 1)
  )

# auto-generate text summary
cat(sprintf("%d mentioned twice, of which %d report only using 1 water point (and thus we would classify as double-counted)\n",
...
# auto-generate text summary
malawi_share <- hh_outside_share %>% dplyr::filter(country == "Malawi") %>% pull(share_wp_with_outside)
uganda_share <- hh_outside_share %>% dplyr::filter(country == "Uganda") %>% pull(share_wp_with_outside)

cat(sprintf("The share of water points in the promoter survey with some users reported to be outside the village is %s%% in Malawi and %s%% in Uganda.\n",
            malawi_share,
            uganda_share))
```
...
very similar to above - 2pp higher share using just 1 water point
```{r}
# filter for HHs in promoter lists
...
# exploratory: does the number of people per water point differ significantly by sample group?
YES for promoter survey water points (and promoter survey user counts), NO for water point census water points (and hh census user counts) 
```{r}
# create wp_users_pm from wp_census
wp_users_pm <- wp_census %>%
  dplyr::select(wp_id, pm_users, country, intervention, sample_group) %>%
  dplyr::filter(!is.na(pm_users))

# create wp_users_census from hh_census by counting distinct households per water point
wp_users_census <- hh_census %>%
  dplyr::group_by(wp_id) %>%
  dplyr::summarise(hh_count = n_distinct(household_id)) %>%
  dplyr::ungroup() %>%
  # join back the water point characteristics
  left_join(
    wp_census %>% dplyr::select(wp_id, country, intervention, sample_group),
    by = "wp_id"
  ) %>%
  dplyr::filter(!is.na(hh_count))

# method 1: convert to unordered factor, then relevel
wp_users_pm <- wp_users_pm %>%
  mutate(sample_group = factor(sample_group, ordered = FALSE),
         sample_group = relevel(sample_group, ref = "Expansion"))

wp_users_census <- wp_users_census %>%
  mutate(sample_group = factor(sample_group, ordered = FALSE),
         sample_group = relevel(sample_group, ref = "Expansion"))

# then run regressions
model_pm_malawi_dsw <- lm(pm_users ~ sample_group, 
                          data = wp_users_pm %>% dplyr::filter(country == "Malawi", intervention == "DSW"))

model_pm_uganda_dsw <- lm(pm_users ~ sample_group, 
                          data = wp_users_pm %>% dplyr::filter(country == "Uganda", intervention == "DSW"))

model_census_malawi_dsw <- lm(hh_count ~ sample_group, 
                              data = wp_users_census %>% dplyr::filter(country == "Malawi", intervention == "DSW"))

model_census_uganda_dsw <- lm(hh_count ~ sample_group, 
                              data = wp_users_census %>% dplyr::filter(country == "Uganda", intervention == "DSW"))

# view promoter survey results
stargazer(model_pm_malawi_dsw, model_pm_uganda_dsw,
          type = "text",
          title = "Number of users per water point by sample group (Promoter Survey)",
          column.labels = c("Malawi DSW", "Uganda DSW"),
          dep.var.labels = "PM Users",
          covariate.labels = c("Footprint", "ILC"),
          keep.stat = c("n", "rsq"))

# view household census results
stargazer(model_census_malawi_dsw, model_census_uganda_dsw,
          type = "text",
          title = "Number of users per water point by sample group (Household Census)",
          column.labels = c("Malawi DSW", "Uganda DSW"),
          dep.var.labels = "Census HH Count",
          covariate.labels = c("Footprint", "ILC"),
          keep.stat = c("n", "rsq"))
```

# exploratory: For how many water points do we have less than 4 households in the monitoring? 
```{r}
wp_monitoring_counts <- hh_survey %>%
  dplyr::filter(promoter_surveyed == "TRUE") %>%
  # dplyr::filter(survey == "Monitoring Survey") %>%
  group_by(wp_id) %>%
  summarise(n_households = n()) %>%
  summarise(share_under_4 = mean(n_households < 4))

wp_monitoring_counts
```

# exploratory: Among households that self report using a promoter survey water point, do those that are in the monitoring survey have higher chlorination? YES
```{r}
promoter_wp_households <- hh_survey %>%
  dplyr::filter(promoter_surveyed == 1) 
...
uganda_model <- lm_robust(disctcr_02 ~ survey, 
                          data = uganda_promoter_wp,
                          clusters = wp_id)

malawi_model <- lm_robust(disctcr_02 ~ survey, 
                          data = malawi_promoter_wp,
                          clusters = wp_id)

# convert to regular lm objects for stargazer
uganda_lm <- lm(disctcr_02 ~ survey, data = uganda_promoter_wp)
malawi_lm <- lm(disctcr_02 ~ survey, data = malawi_promoter_wp)

stargazer(uganda_lm, malawi_lm,
          type = "text",
          title = "Association between survey and Chlorination",
...
## Does the same hold when we add the households that should have been in the monitoring survey? YES
```{r}
promoter_wp_households_adjusted <- hh_survey %>%
  dplyr::filter(promoter_surveyed == 1) %>%
  mutate(
    rank = as.numeric(rank),
    survey = as.character(survey),  # Convert to character first
...
uganda_promoter_wp <- promoter_wp_households_adjusted %>%
  dplyr::filter(country == "Uganda")

malawi_promoter_wp <- promoter_wp_households_adjusted %>%
  dplyr::filter(country == "Malawi")
...
## What about if we combine HHs from both surveys, but compare by WP presence in promoter survey?
This is the same as the regression 
```{r}
chlor_tests <- hh_survey %>%
  group_by(country, promoter_surveyed) %>%
  dplyr::filter(!is.na(promoter_surveyed)) %>%
  summarize(
    chlorination_fcr = mean(discfcr_02, na.rm = TRUE),
    chlorination_tcr = mean(disctcr_02, na.rm = TRUE),
    n = n()
  )
...
# testing SE with village clustering
```{r}
compute_se_village_clustered <- function(values, village_ids, ses = NULL, include_within = FALSE) {
  values <- unlist(values)
  village_ids <- unlist(village_ids)
  ses <- unlist(ses)
  
  # create dataframe for easier manipulation
  df <- data.frame(value = values, village = village_ids)
  df <- df[!is.na(df$value) & !is.na(df$village), ]
  
  if (nrow(df) == 0) {
    return(tibble(mean_val = NA_real_, se = NA_real_))
  }
  
  # calculate village-level means
  village_means <- df %>%
    group_by(village) %>%
    summarise(village_mean = mean(value, na.rm = TRUE), .groups = "drop")
  
  K_villages <- nrow(village_means)
  
  if (K_villages <= 1) {
    between_var <- 0
  } else {
    overall_mean <- mean(village_means$village_mean, na.rm = TRUE)
    between_var <- sum((village_means$village_mean - overall_mean)^2, na.rm = TRUE) / 
                   (K_villages * (K_villages - 1))
  }
  
  # calculate within-village variance (if needed)
  within_var <- 0
  if (include_within && length(ses) == length(values)) {
    # add within-village variance from provided SEs
    df$se <- ses[!is.na(values) & !is.na(village_ids)]
    within_village_var <- df %>%
      group_by(village) %>%
      summarise(village_within_var = sum(se^2, na.rm = TRUE) / n()^2, .groups = "drop") %>%
      summarise(avg_within_var = mean(village_within_var, na.rm = TRUE)) %>%
      pull(avg_within_var)
    
    within_var <- within_village_var
  }
...
```{r}
hh_count <- hh_census %>%
  group_by(country, sample_group, intervention, village_id, wp_id) %>%  # ADD village_id
  summarise(
    hh_count = n_distinct(household_id),
    .groups = "drop"
  ) %>%
  ...
  summarise(
    wp_values = list(hh_count[!is.na(hh_count)]),
    wp_villages = list(village_id[!is.na(hh_count)]),  # ADD THIS
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(tmp = list(compute_se_village_clustered(wp_values, wp_villages, include_within = FALSE))) %>%  # CHANGE FUNCTION
  unnest_wider(tmp, names_sep = "_") %>%
  rename(
    avg_hh_count = tmp_mean_val,
    se_hh_count = tmp_se
...
hh_stats <- hh_survey %>%
  ...
  left_join(hh_size_from_census, by = c("country", "sample_group", "intervention", "village_id", "wp_id")) %>%  # ADD village_id
  group_by(country, sample_group, intervention) %>%
  summarize(
    wp_hh_sizes = list(hh_size[!is.na(hh_size)]),
    wp_hh_villages = list(village_id[!is.na(hh_size)]),  # ADD THIS
    wp_rates_tcr = list(chl_rate_tcr[!is.na(chl_rate_tcr)]),
    wp_rates_fcr = list(chl_rate_fcr[!is.na(chl_rate_fcr)]),
    wp_villages_tcr = list(village_id[!is.na(chl_rate_tcr)]),  # ADD THIS
    wp_villages_fcr = list(village_id[!is.na(chl_rate_fcr)]),  # ADD THIS
    wp_ses_tcr = list(se_chl_rate_tcr[!is.na(se_chl_rate_tcr)]),
    wp_ses_fcr = list(se_chl_rate_fcr[!is.na(se_chl_rate_fcr)]),
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(
    hh_tmp = list(compute_se_village_clustered(wp_hh_sizes, wp_hh_villages, include_within = FALSE)),  # CHANGE FUNCTION
    chl_tmp_tcr = list(compute_se_village_clustered(wp_rates_tcr, wp_villages_tcr, wp_ses_tcr, include_within = TRUE)),  # CHANGE FUNCTION
    chl_tmp_fcr = list(compute_se_village_clustered(wp_rates_fcr, wp_villages_fcr, wp_ses_fcr, include_within = TRUE))  # CHANGE FUNCTION
  ) %>%
  unnest_wider(hh_tmp, names_sep = "_") %>%
  unnest_wider(chl_tmp_tcr, names_sep = "_") %>%
  unnest_wider(chl_tmp_fcr, names_sep = "_") %>%
...
# water point type table
table12_village_clustered <- create_summary_table(hh_count, hh_stats, country_totals)
table12_village_clustered

# country estimates table
country_estimates_table12_village_clustered <- compute_country_totals(table12_village_clustered)
country_estimates_table12_village_clustered
```