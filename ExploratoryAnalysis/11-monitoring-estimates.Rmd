...
## rank dataset (from randomization)
```{r}
pm_survey_clean <-
  read_rds(
    file.path(
...
# if this code is not commented out, we include households that would have been selected for the monitoring survey but had already been present in the household survey
# hh_survey <- hh_survey %>%
#   mutate(
...
```{r}
# function to generate latex table from summary table and country totals
...
## households (11)
```{r}
# renaming intervention in hh census for consistency
# hh_census <- hh_census %>%
...
ideally T1 numbers would automatically come from T1 calculations file
...
521 mentioned twice, of which 341 report only using 1 water point (and thus we would classify as double-counted)
```{r}
