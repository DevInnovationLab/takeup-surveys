...
TO DO 
- what if i classify by the village sample group (impact on country totals and hence estimates)
...
below, classifying by the sample group of the water point
```{r}
# malawi 
...
### looking into whether classifying as expansion/footprint by water point vs village makes a difference
```{r}
...
hh_stats <- hh_survey %>%
  # use household survey and do not filter for water point being in promoter survey 
  dplyr::filter(survey == "Household Survey") %>%
  group_by(country, sample_group, intervention, wp_id) %>%
  summarize(
    hh_size = mean(hh_members, na.rm = TRUE),
