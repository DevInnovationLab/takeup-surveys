...
# Use country totals tables directly
estimates <- country_estimates_table1 %>%
  select(country, table1 = indiv_access_popn) %>%
  left_join(
    country_estimates_table1_exout %>% select(country, table1_exout = indiv_access_popn),
    by = "country"
  ) %>%
  left_join(
    country_estimates_table12_exout %>% select(country, table12_exout = indiv_access_popn),
    by = "country"
  ) %>%
  left_join(
    country_estimates_table10_exout %>% select(country, table10_exout = indiv_access_popn),
    by = "country"
  ) %>%
  left_join(
    country_estimates_table14 %>% select(country, table14 = indiv_access_popn),
    by = "country"
  ) %>%
  left_join(
    country_estimates_table13 %>% select(country, table13 = indiv_access_popn),
    by = "country"
  ) %>%
  left_join(
    country_estimates_table8 %>% select(country, table8 = indiv_access_popn),
    by = "country"
  ) %>%
  left_join(
    country_estimates_table12 %>% select(country, table12 = indiv_access_popn),
    by = "country"
  ) %>%
  left_join(
    country_estimates_table10 %>% select(country, table10 = indiv_access_popn),
    by = "country"
  ) %>%
  mutate(pm_to_census_ratio = table8 / table1)

# Then use the existing waterfall function (with fixed column names as before)
...
create_waterfall <- function(country_name, country_data) {
  ...
  # Calculate positions using a loop for clarity
  df <- df %>%
    mutate(
      id = row_number(),
      type = case_when(
  ...
  for(i in 1:nrow(df)) {
    if(df$type[i] == "total" && i == 1) {
      # First total: start at 0
      df$start[i] <- 0
      df$end[i] <- df$amount_rounded[i]
      running_total <- df$amount_rounded[i]
    } else if(df$type[i] == "total" && i == nrow(df)) {
      # Last total: start at 0, end at full value
      df$start[i] <- 0
      df$end[i] <- df$amount_rounded[i]
    } else {
      # Middle bars: float at running total
      df$start[i] <- running_total
      df$end[i] <- running_total + df$amount_rounded[i]
      running_total <- df$end[i]
    }
  }
...
## sample: EA list vs promoter survey
### uganda
```{r}
ea_promoters_uganda <-
  ea_promoters_uganda %>%
