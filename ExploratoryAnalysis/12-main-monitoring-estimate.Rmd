...
# with CIs - weigh each WP equally
## function - svydesign
```{r}
...
generate_table <- function(users_with_ci, chlorination_tcr_ci, chlorination_fcr_ci,
  ...
  display_table <- table_data %>%
    mutate(
      ...
      `Avg # HH (SE)` = sprintf("%.2f (%.2f)", avg_hh_count, se_hh_count),
      `Avg # Members (SE)` = sprintf("%.2f (%.2f)", avg_hh_size, se_hh_size),
      `# people per WP (SE)` = sprintf("%.2f (%.2f)", indiv_access_sample, se_indiv_access_sample),
      `Country #WP` = format(country_total, big.mark = ",", scientific = FALSE, trim = TRUE),
      `People using DSW/ILC (95% CI)` = sprintf("%s (%s, %s)",
                                                 format(round(indiv_access_popn), big.mark = ",", scientific = FALSE, trim = TRUE),
                                                 format(round(ci_lower_indiv_access_popn), big.mark = ",", scientific = FALSE, trim = TRUE),
                                                 format(round(ci_upper_indiv_access_popn), big.mark = ",", scientific = FALSE, trim = TRUE)),
      `TCR>0.2ppm (95% CI)` = sprintf("%.1f%% (%.1f%%, %.1f%%)", avg_chl_rate_tcr * 100, ci_lower_chl_rate_tcr * 100, ci_upper_chl_rate_tcr * 100),
      `FCR>0.2ppm (95% CI)` = sprintf("%.1f%% (%.1f%%, %.1f%%)", avg_chl_rate_fcr * 100, ci_lower_chl_rate_fcr * 100, ci_upper_chl_rate_fcr * 100),
      `# WP` = ifelse(is.na(n_wp), "-", n_wp),
      `# HH` = ifelse(is.na(n_hh), "-", n_hh)
    ) %>%
    select(`Sample Group`, `Avg # HH (SE)`, `Avg # Members (SE)`, `# people per WP (SE)`,
           `Country #WP`, `People using DSW/ILC (95% CI)`, `TCR>0.2ppm (95% CI)`, 
           `FCR>0.2ppm (95% CI)`, `# WP`, `# HH`, country)
  
  if(output %in% c("kable", "both")) {
    # Generate kable table
    kable_output <- display_table %>%
      knitr::kable(
        format = "html",
        caption = caption,
        escape = FALSE
...
```{r}
generate_latex_table <- function(users_with_ci, chlorination_tcr_ci, chlorination_fcr_ci,
                                  country_totals_with_ci, wp_sample, hh_sample,
                                  caption = "User estimates",
                                  label = "beneficiaries") {
...
generate_latex_table <- function(users_with_ci, chlorination_tcr_ci, chlorination_fcr_ci,
  ...
  cat("\\hline\\hline\n")
  cat("\\multicolumn{8}{p{\\linewidth}}{\\footnotesize Notes:}\\\\\n")
  cat("\\end{tabular}\n")
  cat("\\end{table}\n")
}
...
we have 1 data point per hh, so we weigh by number of observations per water point to get weight 1 per water point
```{r}
# prepare weights = 1/n per water point - combined across sample_groups
hh_size_data_weighted <- hh_survey %>%
  mutate(intervention = wp_intervention) %>%
  select(-wp_intervention) %>%
  dplyr::filter(survey == "Monitoring Survey") %>%
...
## chlorination rates (combined across sample_groups)
```{r}
...
# difference in chlorination between types of DSW
```{r}
