...
table %>% select(country, intervention, n_wp, n_hh_count, n_wp_chl, n_hh, pop_points, popn_users, popn_users_lb, popn_users_ub, tcr, tcr_ub, tcr_lb, fcr, fcr_ub, fcr_lb)
...
## latex code
```{r}
generate_intervention_latex_table <- function(table,
                                              caption = "Monitoring estimates by intervention (DSW and ILC combined across sample groups)",
                                              label = "intervention_estimates") {
  ...
  cat("\\hline \\hline\n")
  cat("& (1) & (2) & (3) & (4) & (5) & (6) & (7) & (8) & (9) \\\\\n")
  cat("& \\multicolumn{3}{c}{\\textbf{Sample}} & \\multicolumn{6}{c}{\\textbf{Country-level estimates}} \\\\\n")
  cat("\\cmidrule(lr){2-4} \\cmidrule(lr){5-10}\n")
  cat(" & \\shortstack{Avg \\# \\\\of HH \\\\ (SE)} &\\shortstack{Avg \\# of \\\\ Members\\\\ (SE)} & \\shortstack{\\# people \\\\ per WP\\\\ (SE)} & \\shortstack{Country \\\\\\#WP} & \\shortstack{People using\\\\ DSW/ILC \\\\ (95\\% CI)} & \\shortstack{\\# WP \\\\ for avg} & \\shortstack{\\# HH \\\\ for chlor} & \\shortstack{TCR>0.2ppm \\\\ (95\\% CI)} & \\shortstack{FCR>0.2ppm \\\\ (95\\% CI)} \\\\\n")
  cat("\\hline\n")
  cat("\\textbf{Malawi} & & & & & & & & & \\\\\n")
  cat("\\hline\n")
  ...
  for(i in 1:nrow(malawi_data)) {
    row <- malawi_data[i,]
    cat(sprintf("%s & \\shortstack{%.2f \\\\ (%.2f)} & \\shortstack{%.2f \\\\ (%.2f)} & \\shortstack{%.2f \\\\ (%.2f)} & %s & \\shortstack{%s \\\\ (%s, %s)} & %s & %s & \\shortstack{%.1f\\%% \\\\ (%.1f\\%%, %.1f\\%%)} & \\shortstack{%.1f\\%% \\\\ (%.1f\\%%, %.1f\\%%)} \\\\\n",
                row$intervention,
                row$hh_count, row$se_hh_count,
                row$hh_size, row$se_hh_size,
                row$per_wp_users, row$se_per_wp_users,
                format(row$pop_points, big.mark = ",", scientific = FALSE, trim = TRUE),
                format(round(row$popn_users), big.mark = ",", scientific = FALSE, trim = TRUE),
                format(round(row$popn_users_lb), big.mark = ",", scientific = FALSE, trim = TRUE),
                format(round(row$popn_users_ub), big.mark = ",", scientific = FALSE, trim = TRUE),
                row$n_wp,
                row$n_hh,
                row$tcr * 100,
                row$tcr_lb * 100,
  ...
  # Malawi total row (empty for manual entry)
  cat("\\textbf{Total} & - & - & - & - & & - & - & - & - \\\\\n")
  ...
  # Uganda total row (empty for manual entry)
  cat("\\textbf{Total} & - & - & - & - & & - & - & - & - \\\\\n")
  
  cat("\\hline\\hline\n")
  cat("\\multicolumn{10}{p{\\linewidth}}{\\footnotesize Notes:}\\\\\n")
  cat("\\end{tabular}\n")
  cat("\\end{sidewaystable}\n")
}

# Usage:
generate_intervention_latex_table(
  table = table,
  caption = "Monitoring estimates by intervention (DSW and ILC combined across sample groups)",
  label = "intervention_estimates"
)
