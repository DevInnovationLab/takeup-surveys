...
```{r}
# prepare data for regression - focusing on DSW only
reg_data <- hh_survey %>%
  mutate(intervention = wp_intervention) %>%
  dplyr::filter(survey == "Monitoring Survey") %>%
  dplyr::filter(wp_func == "TRUE") %>%
  dplyr::filter(intervention == "DSW") %>%
  dplyr::filter(sample_group %in% c("Expansion", "Footprint", "ILC")) %>%
  # make sample_group a factor with Expansion as reference
  mutate(
    sample_group = factor(sample_group, levels = c("Expansion", "Footprint", "ILC")),
    disctcr_02_num = as.numeric(disctcr_02)  # convert logical to numeric for regression
  )

# Malawi regression
model_malawi <- lm(
  disctcr_02_num ~ sample_group,
  data = reg_data %>% dplyr::filter(country == "Malawi")
)
...
coeftest(model_malawi, vcov = vcovCL(model_malawi, cluster = reg_data %>% dplyr::filter(country == "Malawi") %>% pull(wp_id)))

# extract clustered SEs for stargazer
clustered_se_malawi <- sqrt(diag(vcovCL(model_malawi, cluster = reg_data %>% dplyr::filter(country == "Malawi") %>% pull(wp_id))))

# Uganda regression
model_uganda <- lm(
  disctcr_02_num ~ sample_group,
  data = reg_data %>% dplyr::filter(country == "Uganda")
)

coeftest(model_uganda, vcov = vcovCL(model_uganda, cluster = reg_data %>% dplyr::filter(country == "Uganda") %>% pull(wp_id)))

clustered_se_uganda <- sqrt(diag(vcovCL(model_uganda, cluster = reg_data %>% dplyr::filter(country == "Uganda") %>% pull(wp_id))))

# stargazer with clustered SEs
stargazer(
  model_malawi, model_uganda,
  type = "text",
  se = list(clustered_se_malawi, clustered_se_uganda),
  title = "Chlorination rates by sample group (DSW only)",
  column.labels = c("Malawi", "Uganda"),
  dep.var.labels = "TCR >= 0.2 ppm",
  covariate.labels = c("Footprint", "ILC", "Constant (Expansion)"),
  keep.stat = c("n", "adj.rsq"),
  notes = c("Standard errors clustered at water point level in parentheses.",
            "*** p<0.01, ** p<0.05, * p<0.1",
            "Reference category: Expansion villages"),
  notes.align = "l"
)
```
