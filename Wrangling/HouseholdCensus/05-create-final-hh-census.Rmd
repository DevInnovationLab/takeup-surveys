...
hhc_primarywp <-
  ...
  transmute(
    ...
    wp_dsw = coalesce(wp_dsw_pl %>% as.numeric %>% as.logical, wp_dsw),
    wp_ilc = coalesce(wp_ilc_pl %>% as.numeric %>% as.logical, wp_ilc == "Yes")
  ) %>%
  mutate(
    wp_id_c = case_when(
...
hhc_promoter <-
  ...
  transmute(
    ...
    promoter_list_mentions = household_id %>%
      map_int(~sum(pm_survey_households$household_id == .x))
  )
```
...
indexes <-
  ...
  mutate(
    wp_intervention_type = case_when(
      # In this village, all of the water points within the boundaries (ILC taps) were not functioning. Therefore, all of the households were using water points outside of the village. The enumerators indicated which of these had ILC/DSW in the survey
      village_id == "2040920077" & wp_name_o %in% c(
        "Zumulu school borehole",
        "Zumulu borehole",
        "Zumulu borehole/ Zumulu CCAP",
        "Nkopoti borehole",
        "Mjigo wa pa school",
        "Mpopi wa pa school",
        "Mjigo wa ku Zumulu Primary",
        "Mjigo wa ku Zumulu Primary school",
        "Khuzumulu borehole"
      ) ~ "DSW",
      village_id == "2040920077" ~ "Non-program",
      # In this village, the water point with DSW is at a far corner, so many households use water points with DSW in other villages. The enumerators indicated which of these have DSW in the survey
      village_id == "2101405067" & wp_name_o %in% c(
        "Malidadi borehole",
        "Mjigo Wa Pa Malidad",
        "Mjigo Wa Pa Malidadi",
        "Mayilosi Borehole",
        "Mjigo Wa Kwa Anamukutako",
        "Mjigo wa Kwa Namukutaku",
        "Mjigo Wa Kwa Ndaje",
        "Mjigo Wa Pa Chipatala",
        "Mjigo wa Mmudzi Mwa a Taulo"
      ) ~ "DSW",
      village_id == "2101405067" & wp_name_o %in% c(
        "Mpambana tap",
        "Mpopi Wa Kwa Mpambana",
        "Mpopi Wa Kwa Mukwali",
        "Mpopi Wa Kwa Mukwalu",
        "Mpopi Wa Kwa Mukwalu Village"
      ) ~ "ILC water point",
      # In this village, the only water point within the boundaries (a tap with DSW) was not functioning. Therefore, all of the households were using water points outside of the village. The enumerators indicated which of these have DSW in the survey
      village_id == "2101220134" & !is.na(wp_name_o) ~ "DSW",
      village_id == "2101220134" &  is.na(wp_name_o) ~ NA,
    ...
    wp_intervention = case_when(
       is.na(wp_intervention) & wp_intervention_type == "DSW" ~ "DSW",
       is.na(wp_intervention) & str_detect(wp_intervention_type, "ILC") ~ "ILC",
       # Households using piped water into the house
       village_id == "2101220134" & is.na(wp_name_o) ~ NA,
