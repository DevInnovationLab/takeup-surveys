...
wp <-
  ...
  transmute(
    ...
    hh_ilc = ilc,
    wp_func,
    wp_intervention_type = intervention_type %>% as.factor,
...
villages <-
  ...
  mutate(
    sample_group_combined = if_else(sample_group == "ILC", "ILC", "DSW")
  )
```
...
hhc_meta <-
  ...
  select(
    household_id, 
    formdef_version,
    comments, endcomment, review_comments
  )
```
