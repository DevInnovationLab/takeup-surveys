...
For `wp_name_1`, enumerators can input a string, so we compute the average string
distance between water sources. We consider a `wp_name_1` to be a match if
...
compare_wps <-
  ...
  group_by(mch_id) %>% 
   mutate(
    ...
    match_wp_name_1 = ifelse(is.na(avg_st_dist), F, (avg_st_dist <= 0.3))
  ) %>%
  rowwise() %>% 
  mutate(
    pct_match = (sum(
...
wp_chars <- 
  ...
  group_by(wp_id_c) %>%
  mutate(
    wp_promoter_c = any(wp_promoter == "Yes", na.rm = TRUE),
  ...
  group_by(
    ...
    n_obs, 
    n_enum,
    wp_drink, wp_multicompound, wp_func,
    sourcetype, wp_type_o, 
    dsw,
    wp_ilc_evac, wp_ilc_device_c, wp_ilc_c, wp_ilc_evac_c
  ) %>%
  ...
  select(
    ...
    n_enum,
    wp_drink, wp_multicompound, wp_func,
    sourcetype, sourcetype_simp, wp_type_o,
    intervention_type,
    dsw, ilc_wp, ilc_wcp,
    wp_ilc_evac, wp_ilc_device_c, wp_ilc_c, wp_ilc_evac_c
  ) %>%
  mutate(ilc = ilc_wp | ilc_wcp) %>%
...
### Select only spotcheck variables
...
spotcheck <-
  ...
  select(
    ...
    spotcheck_enum_name = enumerator_name,
    spotcheck_time = submissiondate,
    wp_func_reason:ilc_stock_whynot_o
  )
```

There should be only one observation per WP, but there are `r spotcheck %>% group_by(wp_id_c) %>% summarise(n = n()) %>% dplyr::filter(n > 1) %>% nrow` cases of duplicate observations. 

If all the information is the same, there are just two observations, we will choose the observation randomly.
...
In `r spotcheck_conflicts %>% pull(wp_id_c) %>% n_distinct` water points, there is some conflicting information. These are water points that were revisited. We will keep only the last observation in these cases
...
If that's not the case, select the most common set of observations
...
meter <-
  ...
  select(
    ...
    meter_intro:metertcr_timestamp6,
...
cw_nocl <-
  ...
  select(
    ...
    discid_nocl:disctcr_nocl_dispose_nt,
    -contains(c("materials", "insert", "rinse", "_nt", "_read")),
    -ends_with(c("check", "confirm"))
  ) %>%
  unique
...
wp_collapsed <-
  ...
  left_join(spotcheck) %>%
  left_join(cw_unique) %>%
  left_join(meter_unique) %>%
  left_join(cw_nocl_unique) %>%
  left_join(meter_nocl)
```
